<TeXmacs|1.0.7.20>

<style|generic>

<\body>
  <\render-exercise|Exercise 1.12>
    <render-footnote|*|Creative Commons <image|<tuple|<#>|png>|1.20cm|0.42cm||>
    2013, Lawrence X. Amlord (<with|font|heiti|\<#989C\>\<#4E16\>\<#654F\>>,
    aka <with|font|heiti|\<#989C\> \<#5E8F\>>).>

    The following pattern of numbers is called <em|Pascal's triangle>.

    <math|<stack|<tformat|<table|<row|<cell|>|<cell|>|<cell|>|<cell|>|<cell|1>|<cell|>|<cell|>|<cell|>|<cell|>>|<row|<cell|>|<cell|>|<cell|>|<cell|1>|<cell|>|<cell|1>|<cell|>|<cell|>|<cell|>>|<row|<cell|>|<cell|>|<cell|1>|<cell|>|<cell|2>|<cell|>|<cell|1>|<cell|>|<cell|>>|<row|<cell|>|<cell|1>|<cell|>|<cell|3>|<cell|>|<cell|3>|<cell|>|<cell|1>|<cell|>>|<row|<cell|1>|<cell|>|<cell|4>|<cell|>|<cell|6>|<cell|>|<cell|4>|<cell|>|<cell|1>>>>>>

    The numbers at the edge of the triangle are all <math|1>, and each number
    inside the triangle is the sum of the two numbers above it.<\footnote>
      The elements of Pascal's triangle are called the <em|binomial
      coefficients>, because the <math|n>th row consists of the coefficients
      of the terms in the expansion of <math|(x+y)<rsup|n>>. This pattern for
      computing the coefficients appeared in Blaise Pascal's 1653 seminal
      work on probability theory, <em|Traité du triangle arithmétique>.
      According to Knuth (1973), the same pattern appears in the <em|Szu-yuen
      Y<math|<wide|u|\<ddot\>>>-chien>(``The Precious Mirror of the Four
      Elements''), published by the Chinese mathematician Chu Shih-chieh in
      1303, in the works of the twelfth-century Persian poet and
      mathematician Omar Khayyam, and in the works of the twelfth-century
      Hindu mathematician Bháscara Áchárya.
    </footnote> Write a procedure that computes elements of Pascal's triangle
    by means of a recursive process.

    <\answer>
      \;

      The distribution of elements inside the triangle implies that the
      problem might become easier if we denote their position in an
      appropriate coordinates. Figure <reference|Exercise_1.12-figure_1><\float|float|tbh>
        <big-figure|<label|Exercise_1.12-figure_1><with|gr-mode|<tuple|group-edit|group-ungroup>|gr-frame|<tuple|scale|1cm|<tuple|0.5gw|0.5gh>>|gr-geometry|<tuple|geometry|1par|0.6par>|gr-grid|<tuple|empty>|gr-grid-old|<tuple|cartesian|<point|0|0>|1>|gr-edit-grid-aspect|<tuple|<tuple|axes|none>|<tuple|1|none>|<tuple|10|none>>|gr-edit-grid|<tuple|empty>|gr-edit-grid-old|<tuple|cartesian|<point|0|0>|1>|gr-dash-style|10|gr-auto-crop|true|<graphics||<gr-group|<with|arrow-end|\<gtr\>|<line|<point|-1.5|2.5>|<point|-4.5|-0.5>>>|<with|dash-style|10|<line|<point|-5|0.2>|<point|2.0|0.2>>>|<text-at|<math|5>|<point|-4.38813|0.0630739999999999>>|<text-at|<with|font-shape|italic|row>|<point|-4.2|-0.6>>|<point|-3.81431|0.187905>|<text-at|<math|4>|<point|-4.048786|0.459409>>|<with|dash-style|10|<line|<point|1.2|3>|<point|-2.8|-1.0>>>|<point|-3.45447|0.526574>|<text-at|<math|3>|<point|-3.644925|0.79125>>|<point|-3.11581|0.88641>|<text-at|<math|2>|<point|-3.30875|1.19125>>|<point|-2.75597|1.22508>|<text-at|<math|1>|<point|-2.9|1.5486>>|<point|-2.4173|1.58492>|<with|arrow-end|\<gtr\>|<line|<point|-2.8|2.0>|<point|1.8|2.0>>>|<text-at|<math|O>|<point|-2.3|2.2>>|<point|-1.3|2>|<text-at|<math|1>|<point|-1.35711987006992|2.211674>>|<text-at|<math|><math|<stack|<tformat|<table|<row|<cell|>|<cell|>|<cell|>|<cell|>|<cell|1>|<cell|>|<cell|>|<cell|>|<cell|>>|<row|<cell|>|<cell|>|<cell|>|<cell|1>|<cell|>|<cell|1>|<cell|>|<cell|>|<cell|>>|<row|<cell|>|<cell|>|<cell|1>|<cell|>|<cell|2>|<cell|>|<cell|1>|<cell|>|<cell|>>|<row|<cell|>|<cell|1>|<cell|>|<cell|3>|<cell|>|<cell|3>|<cell|>|<cell|1>|<cell|>>|<row|<cell|1>|<cell|>|<cell|4>|<cell|>|<cell|6>|<cell|>|<cell|4>|<cell|>|<cell|1>>>>>>|<point|-3.2|0.8>>|<point|-0.6|2>|<text-at|<math|2>|<point|-0.7|2.2>>|<point|0.207385|1.98708>|<text-at|<math|3>|<point|0.13443476206305|2.2>>|<point|0.990558|1.98708>|<text-at|<math|4>|<point|0.951214|2.18492>>|<text-at|<with|font-shape|italic|column>|<point|1.3|1.6>>>>>|A
        way to expressing Pascal's triangle in the coordinates.>
      </float> shows a way to expressing Pascal's triangle in the
      coordinates, in which the horizonal axis denotes the column an element
      locates and the row of it can be labeled by an downward inclined axis.
      Hence, the element <math|6> inside the triangle above is appointed with
      the coordinate <math|<around*|(|3,5|)>>.<new-line>
      <hspace|2ex>Obviously, elements which are not in Pascal's triangle are
      all <math|0>, that is

      <math|\<bullet\>> If the <with|font-shape|italic|column> coordinate of
      an element is smaller than <math|1>, the value of it would be <math|0>.

      <math|\<bullet\>> If the <with|font-shape|italic|row> coordinate of an
      element is smaller than its <with|font-shape|italic|column> coordinate,
      the value of it would be <math|0>.

      <hspace|3ex>To those that are the elements of the triangle, we observe
      that

      <math|\<bullet\>> The edge of the triangle are those elements whose
      <with|font-shape|italic|column> coordinate are either equal to <math|1>
      or equal to its <with|font-shape|italic|row> coordinate.

      <math|\<bullet\>> Any number inside the triangle with the coordinates
      <math|<around*|(|c,r|)>> is the sum of the two elements above it, whose
      coordinates are <math|<around*|(|c-1,r-1|)>> and
      <math|<around*|(|c,r-1|)>> respectively.

      We can easily translate this description into a recursive procedure:

      <\scm-code>
        (define (pascal-triangle row column)

        \ \ (cond ((or (\<less\> cloumn 1) (\<less\> row column)) 0)

        \ \ \ \ \ \ \ \ ((or (= column 1) (= row column)) 1)

        \ \ \ \ \ \ \ \ (else (+ (pascal-triangle (- row 1) (- column 1))

        \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ (pascal-triangle (- row 1)
        column)))))
      </scm-code>
    </answer>
  </render-exercise>
</body>

<\initial>
  <\collection>
    <associate|font-base-size|11>
  </collection>
</initial>

<\references>
  <\collection>
    <associate|35|<tuple|?|?>>
    <associate|Exercise_1.12-figure_1|<tuple|1|?>>
    <associate|auto-1|<tuple|1|1>>
    <associate|footnote-|<tuple|?|1>>
    <associate|footnote-*|<tuple|?|1>>
    <associate|footnote-1|<tuple|1|1>>
    <associate|footnote-3|<tuple|?|?>>
    <associate|footnote-35|<tuple|?|1>>
    <associate|footnote-\\underline|<tuple|?|?>>
    <associate|footnote-_|<tuple|?|?>>
    <associate|footnote-_3|<tuple|?|?>>
    <associate|footnote-_35|<tuple|?|?>>
    <associate|footnr-1|<tuple|1|1>>
  </collection>
</references>

<\auxiliary>
  <\collection>
    <\associate|figure>
      <tuple|normal|A way to expressing Pascal's triangle in the
      coordinates.|<pageref|auto-1>>
    </associate>
  </collection>
</auxiliary>