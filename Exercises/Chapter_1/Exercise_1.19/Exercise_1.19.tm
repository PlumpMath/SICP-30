<TeXmacs|1.0.7.19>

<style|generic>

<\body>
  <\render-exercise|Exercise 1.19>
    <render-footnote|*|Creative Commons <image|<tuple|<#>|png>|1.20cm|0.42cm||>
    2013, Lawrence R. Amlord(<with|font|heiti|\<#989C\>\<#4E16\>\<#654F\>>).>There
    is a clever algorithm for computing the Fibonacci numbers in a
    logarithmic number of steps. Recall the transformation of the state
    variables <math|a> and <math|b> in the <scm|fib-iter> process of section
    1.2.2: <math|a \<leftarrow\>a+b> and <math|b\<leftarrow\>a>. Call this
    transformation <math|T>, and observe that applying <math|T> over and over
    again <math|n> times, starting with <math|1> and <math|0>, produces the
    pair <math|Fib <around*|(|n+1|)>> and <math|Fib <around*|(|n|)>>. In
    other words, the Fibonacci numbers are produced by applying
    <math|T<rsup|n>>, the <math|n>th power of the transformation <math|T>,
    starting with the pair <math|(1,0)>. Now consider <math|T> to be the
    special case of <math|p= 0> and <math|q= 1> in a family of
    transformations <math|T<rsub|p q>>, where <math|T<rsub|p q>> transforms
    the pair <math|(a,b)> according to <math|a\<leftarrow\>b*q+a*q+a*p> and
    <math|b\<leftarrow\>b*p+a*q>. Show that if we apply such a transformation
    <math|T<rsub|p q>> twice, the effect is the same as using a single
    transformation <math|T<rsub|p<rprime|'>q<rprime|'>>> of the same form,
    and compute <math|p<rprime|'>> and <math|q<rprime|'>> in terms of
    <math|p> and <math|q>. This gives us an explicit way to square these
    transformations, and thus we can compute <math|T<rsup|n>> using
    successive squaring, as in the <scm|fast-expt> procedure. Put this all
    together to complete the following procedure, which runs in a logarithmic
    number of steps:<\footnote>
      This exercise was suggested to us by Joe Stoy, based on an example in
      Kaldewaij 1990.
    </footnote>

    <\scm-code>
      (define (fib n)

      \ \ (fib-iter 1 0 0 1 n))

      (define (fib-iter a b p q count)

      \ \ (cond ((= count 0) b)

      \ \ \ \ \ \ \ \ ((even? count)

      \ \ \ \ \ \ \ \ \ (fib-iter a

      \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ b

      \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \<less\>??\<gtr\> \ \ \ ; compute
      p'

      \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \<less\>??\<gtr\> \ \ \ ; compute
      q'

      \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ (/ count 2)))

      \ \ \ \ \ \ \ \ (else (fib-iter (+ (* b q) (* a q) (* a p))

      \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ (+ (* b p) (* a q))

      \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ p

      \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ q

      \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ (- count 1)))))
    </scm-code>

    <\answer>
      By reading the description above, we may easily conclude that the key
      point to solving this problem relys on how to represent
      <math|p<rprime|'>> and <math|q<rprime|'>> in terms of <math|p> and
      <math|q>. With observation, we may immediately draw out our basic
      strategy in a straightforwardly: apply the transformation
      <math|T<rsub|p q>> to the pair <math|<around*|(|a,b|)>> twice, then
      compare the consequence with that of the single transformation
      <math|T<rsub|p<rprime|'>q<rprime|'>>>.\ 

      In order to present the intermediate result in a clear way, we first
      denote the original pair <math|<around*|(|a,b|)>> by
      <math|<around*|(|a<rsub|0>,b<rsub|0>|)>>, that is,

      <\equation*>
        a<rsub|0>=a,b<rsub|0>=b
      </equation*>

      Well, Let's see how the pair <math|<around*|(|a,b|)>> involves while
      being transformed by <math|T<rsub|p q>> for the first time:

      <\eqnarray*>
        <tformat|<table|<row|<cell|a<rsub|1>>|<cell|=>|<cell|b<rsub|0>*q+a<rsub|0>*q+a<rsub|0>*p>>|<row|<cell|>|<cell|=>|<cell|b*q+a*q+a*p>>|<row|<cell|>|<cell|=>|<cell|b*q+a*<around*|(|p+q|)>>>|<row|<cell|>|<cell|>|<cell|>>|<row|<cell|b<rsub|1>>|<cell|=>|<cell|b<rsub|0>*p+a<rsub|0>*q>>|<row|<cell|>|<cell|=>|<cell|b*p+a*q>>>>
      </eqnarray*>

      <\equation*>
        <choice|<tformat|<table|<row|<cell|a<rsub|1>=b*q+a*<around*|(|p+q|)>>>|<row|<cell|b<rsub|1>=b*p+a*q>>>>>
      </equation*>

      Continue in this way, the value of pair
      <math|<around*|(|a<rsub|2>,b<rsub|2>|)>> can be obtained gradually as
      follow:

      <\eqnarray*>
        <tformat|<table|<row|<cell|a<rsub|2>>|<cell|=>|<cell|b<rsub|1>*q+a<rsub|1>*<around*|(|p+q|)>>>|<row|<cell|>|<cell|=>|<cell|<around*|(|b*p+a*q|)>*q+<around*|(|b*q+a*<around*|(|p+q|)>|)><around*|(|p+q|)>>>|<row|<cell|>|<cell|=>|<cell|b*p*q+a*q<rsup|2>+b*p*q+b*q<rsup|2>+a*<around*|(|p<rsup|2>+2*p*q+q<rsup|2>|)>>>|<row|<cell|>|<cell|=>|<cell|b*<around*|(|q<rsup|2>+2*p*q|)>+a*<around*|(|p<rsup|2>+q<rsup|2>+q<rsup|2>+2*p*q|)>>>|<row|<cell|>|<cell|>|<cell|>>|<row|<cell|b<rsub|2>>|<cell|=>|<cell|b<rsub|1>*p+a<rsub|1>*q>>|<row|<cell|>|<cell|=>|<cell|<around*|(|b*p+a*q|)>*p+<around*|(|b*q+a*<around*|(|p+q|)>|)>*q>>|<row|<cell|>|<cell|=>|<cell|b*p<rsup|2>+a*p*q+b*q<rsup|2>+a*<around*|(|p*q+q<rsup|2>|)>>>|<row|<cell|>|<cell|=>|<cell|b*<around*|(|p<rsup|2>+q<rsup|2>|)>+a*<around*|(|q<rsup|2>+2*p*q|)>>>>>
      </eqnarray*>

      <\equation*>
        <choice|<tformat|<table|<row|<cell|a<rsub|2>=b*<around*|(|q<rsup|2>+2*p*q|)>+a*<around*|(|p<rsup|2>+q<rsup|2>+q<rsup|2>+2*p*q|)>>>|<row|<cell|b<rsub|2>=b*<around*|(|p<rsup|2>+q<rsup|2>|)>+a*<around*|(|q<rsup|2>+2*p*q|)>>>>>>
      </equation*>

      By Comparing the expression of the pair
      <math|<around*|(|a<rsub|2>,b<rsub|2>|)>> above with that of the single
      transformation <math|T<rsub|p<rprime|'>q<rprime|'>>> where

      <\equation*>
        <choice|<tformat|<table|<row|<cell|a<rsub|2>=b*q<rprime|'>+a*<around*|(|p<rprime|'>+q<rprime|'>|)>>>|<row|<cell|b<rsub|2>=b*p<rprime|'>+a*q<rprime|'>>>>>>
      </equation*>

      we can find out the expression of <math|p<rprime|'>> and
      <math|q<rprime|'>> in terms of <math|p> and <math|q> intuitively:

      <\equation*>
        <choice|<tformat|<table|<row|<cell|p<rprime|'>=p<rsup|2>+q<rsup|2>>>|<row|<cell|q<rprime|'>=q<rsup|2>+2*p*q>>>>>
      </equation*>

      Finally, we have to complete the <scm|fib> procedure to bring an end to
      our solution:

      <\scm-code>
        (define (fib n)

        \ \ (fib-iter 1 0 0 1 n))

        (define (fib-iter a b p q count)

        \ \ (cond ((= count 0) b)

        \ \ \ \ \ \ \ \ ((even? count)

        \ \ \ \ \ \ \ \ \ (fib-iter a

        \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ b

        \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ (sum-of-squares p q)
        \ \ \ \ \ \ \ ; compute p'

        \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ (+ (square q) (* 2 p q))
        \ \ \ ; compute q'

        \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ (/ count 2)))

        \ \ \ \ \ \ \ \ (else (fib-iter (+ (* b q) (* a q) (* a p))

        \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ (+ (* b p) (* a q))

        \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ p

        \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ q

        \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ (- count 1)))))
      </scm-code>
    </answer>
  </render-exercise>
</body>

<\references>
  <\collection>
    <associate|footnote-|<tuple|?|?>>
    <associate|footnote-*|<tuple|?|1>>
    <associate|footnote-1|<tuple|1|1>>
    <associate|footnr-1|<tuple|1|1>>
  </collection>
</references>