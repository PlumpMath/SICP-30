<TeXmacs|1.0.7.19>

<style|generic>

<\body>
  <\render-exercise|Exercise 1.30>
    <render-footnote|*|Creative Commons <image|<tuple|<#>|png>|1.20cm|0.42cm||>
    2013, Lawrence R. Amlord(<with|font|heiti|\<#989C\>\<#4E16\>\<#654F\>>).>The
    <scm|sum> procedure above generates a linear recursion. The procedure can
    be rewritten so that the sum is performed iteratively. Show how to do
    this by filling in the missing expressions in the following definition:

    <\scm-code>
      (define (sum term a next b)

      \ \ (define (iter a result)

      \ \ \ \ (if \<less\>??\<gtr\>

      \ \ \ \ \ \ \ \ \<less\>??\<gtr\>

      \ \ \ \ \ \ \ \ (iter \<less\>??\<gtr\> \<less\>??\<gtr\>)))

      \ \ (iter \<less\>??\<gtr\> \<less\>??\<gtr\>))
    </scm-code>

    <\answer>
      To solve this problem, just remember the general strategy we have
      learnt to design a iterative procedure:

      <\itemize-dot>
        <item>Figure out a way to accumulate partial answer.

        <item>Write our a table to analyze precisely:

        <\itemize-minus>
          <item>initialization of first row

          <item>update rules for other rows

          <item>how to know when to stop
        </itemize-minus>

        <item>Translate rules into Scheme code.
      </itemize-dot>

      Let's think about how you might do the summation by hand. An advisable
      way would be drawing a table with several columns, each stands for a
      variable we need to keep track of on every step of computation, say,
      the current index(<scm|a>), the upper bound(<scm|b>) and the result.
      For example, for <scm|(sum-cubes 1 5)>, you would immediately perform
      your computation in a form like Table
      <reference|Exercise_1.30-Table_1><\float|float|tbh>
        <big-table|<label|Exercise_1.30-Table_1><tabular|<tformat|<cwith|1|-1|1|-1|cell-halign|c>|<table|<row|<cell|<scm|a>>|<cell|<scm|result>>|<cell|<scm|b>>>|<row|<cell|1>|<cell|0>|<cell|<math|5>>>|<row|<cell|2>|<cell|1>|<cell|<math|5>>>|<row|<cell|3>|<cell|9>|<cell|<math|5>>>|<row|<cell|4>|<cell|36>|<cell|<math|5>>>|<row|<cell|5>|<cell|100>|<cell|<math|5>>>|<row|<cell|6>|<cell|225>|<cell|<math|5>>>>>>|Raw
        Table for Evaluating <scm|(sum-cube 1 5)> >
      </float>.

      The interface between procedure <scm|sum> and <scm|iter> above suggests
      that we should only keep track of 2 things on each step of our
      computation, although 3 variables are actually demanded. After
      investegating the structure of procedure <scm|sum>, one might notice
      that by using block structure, we can probably keep the third variable
      <scm|b> which set the upper bound outside procedure <scm|iter>.
      Therefore, our original table evolves into a more concise version, as
      is shown in Table <reference|Exercise_1.30-Table_2><\float|float|tbh>
        <big-table|<label|Exercise_1.30-Table_2><with|gr-mode|<tuple|group-edit|move>|gr-frame|<tuple|scale|1cm|<tuple|0.5gw|0.5gh>>|gr-geometry|<tuple|geometry|1par|0.6par>|gr-grid|<tuple|empty>|gr-grid-old|<tuple|cartesian|<point|0|0>|1>|gr-edit-grid-aspect|<tuple|<tuple|axes|none>|<tuple|1|none>|<tuple|10|none>>|gr-edit-grid|<tuple|empty>|gr-edit-grid-old|<tuple|cartesian|<point|0|0>|1>|gr-text-at-halign|center|gr-text-at-valign|center|gr-auto-crop|true|<graphics||<gr-group|<cline|<point|-0.99023340918111|1.17256356499278>|<point|0.93473682497685|1.19234792800376>|<point|0.90976659081889|-1.62743643500722>|<point|-0.99023340918111|-1.62743643500722>>|<text-at|<tabular|<tformat|<cwith|1|-1|1|-1|cell-halign|c>|<table|<row|<cell|<scm|a>>|<cell|<scm|result>>>|<row|<cell|<math|1>>|<cell|<math|0>>>|<row|<cell|<math|2>>|<cell|<math|1>>>|<row|<cell|<math|3>>|<cell|<math|9>>>|<row|<cell|<math|4>>|<cell|<math|36>>>|<row|<cell|<math|5>>|<cell|<math|100>>>|<row|<cell|<math|6>>|<cell|<math|225>>>>>>|<point|-0.71798187590951|-0.28181307051197>>>>>|An
        Updated Table for Evaluating <scm|(sum-cube 1 5)> >
      </float>.\ 

      Now, what we need to do is come up with a rule for how we are going to
      change the values in the table. In particular, to get the next value
      for result. As is shown in Table <reference|Exercise_1.30-Table_3><\float|float|tbh>
        <big-table|<label|Exercise_1.30-Table_3><with|gr-mode|<tuple|group-edit|group-ungroup>|gr-frame|<tuple|scale|1cm|<tuple|0.5gw|0.5gh>>|gr-geometry|<tuple|geometry|1par|0.6par>|gr-grid|<tuple|empty>|gr-grid-old|<tuple|cartesian|<point|0|0>|1>|gr-edit-grid-aspect|<tuple|<tuple|axes|none>|<tuple|1|none>|<tuple|10|none>>|gr-edit-grid|<tuple|empty>|gr-edit-grid-old|<tuple|cartesian|<point|0|0>|1>|gr-text-at-halign|center|gr-text-at-valign|center|gr-auto-crop|true|gr-color|magenta|gr-arrow-end|\<gtr\>|<graphics||<gr-group|<gr-group|<with|color|red|<cline|<point|-3.34881554173832|-0.47774928297394>|<point|-2.09977510252678|-0.47774507209949>|<point|-2.09977510252678|-0.89699695726948>|<point|-3.34881598094986|-0.89699695726948>>>|<with|color|red|text-at-halign|center|text-at-valign|center|<text-at|<scm|(+
        a 1)>|<point|-2.72429554173832|-0.68737928297394>>>>|<with|arrow-end|\<gtr\>|color|red|<spline|<point|-0.655163|-0.22374>|<point|-1.8|0.0>|<point|-2.75067799973541|-0.456574943775632>>>|<with|arrow-end|\<gtr\>|color|red|<spline|<point|-2.11567|-0.731744>|<point|-0.718663182960709|-0.71057679587247>>>|<gr-group|<gr-group|<cline|<point|-0.99023340918111|0.97256356499278>|<point|0.93473682497685|0.99234792800376>|<point|0.90976659081889|-1.82743643500722>|<point|-0.99023340918111|-1.82743643500722>>|<text-at|<tabular|<tformat|<cwith|1|-1|1|-1|cell-halign|c>|<table|<row|<cell|<scm|a>>|<cell|<scm|result>>>|<row|<cell|<math|1>>|<cell|<math|0>>>|<row|<cell|<math|2>>|<cell|<math|1>>>|<row|<cell|<math|3>>|<cell|<math|9>>>|<row|<cell|<math|4>>|<cell|<math|36>>>|<row|<cell|<math|5>>|<cell|<math|100>>>|<row|<cell|<math|6>>|<cell|<math|225>>>>>>|<point|-0.71798187590951|-0.48181307051197>>>|<with|color|magenta|<carc|<point|-0.0296569999999999|-1.56675>|<point|0.370343|-1.56675>|<point|0.170343|-1.66987292757596>>>|<with|color|red|<carc|<point|-0.67145966261921|-0.66327233701139>|<point|-0.37145966261921|-0.66327233701139>|<point|-0.37145966261921|-0.86327233701139>>>|<with|color|red|<carc|<point|-0.67633|-0.2755722>|<point|-0.37633|-0.2755722>|<point|-0.37633|-0.4755722>>>|<with|color|blue|<carc|<point|0.0254992924505101|-0.64558500125781>|<point|0.32549929245051|-0.64558500125781>|<point|0.32549929245051|-0.84558500125781>>>>|<with|color|blue|<cspline|<point|-0.89199645905543|-0.36258051754038>|<point|-0.19199645905543|-0.56258051754038>|<point|0.50800354094457|-0.36258051754038>|<point|-0.19199645905543|-0.16258051754038>>>|<with|arrow-end|\<gtr\>|color|magenta|<spline|<point|1.92719|-2.21342>|<point|1.3|-2.2>|<point|0.974682497684879|-1.5995832782114>|<point|0.403178330466993|-1.55724963619526>>>|<with|arrow-end|\<gtr\>|color|blue|<spline|<point|1.96952|-0.68941>|<point|0.403178330466993|-0.71057679587247>>>|<with|arrow-end|\<gtr\>|color|blue|<spline|<point|0.276177|-0.244907>|<point|2.13889509426396|0.2>|<point|3.72636922873396|-0.435408122767562>>>|<gr-group|<with|color|magenta|<cline|<point|1.94961197380606|-2.02203289323985>|<point|3.04231379812145|-2.02203333774309>|<point|3.04231379812145|-2.34333906601403>|<point|1.94960973673766|-2.34333906601403>>>|<with|color|magenta|text-at-halign|center|text-at-valign|center|<text-at|answer|<point|2.49597499669268|-2.18270273845747>>>>|<with|arrow-end|\<gtr\>|color|dark
        cyan|<spline|<point|2.43519|0.749934>|<point|1.83611119300859|0.8>|<point|1.5|0.4>|<point|0.953515676676809|0.347764254531023>>>|<gr-group|<with|color|blue|<cline|<point|1.97175048592952|-0.44574769546236>|<point|5.01944623272142|-0.44574348458791>|<point|5.01944623272142|-0.8649953697579>|<point|1.97175473913762|-0.8649953697579>>>|<with|color|blue|text-at-halign|center|text-at-valign|center|<text-at|<scm|(+
        (cube a) result)>|<point|3.49560048592952|-0.65537769546236>>>>|<gr-group|<with|color|dark
        cyan|<cline|<point|2.44732898134674|0.95236113454161>|<point|6.12324712263527|0.95236142346871>|<point|6.12324712263527|0.53310953829872>|<point|2.44733099616351|0.53310953829872>>>|<with|color|dark
        cyan|text-at-halign|center|text-at-valign|center|<text-at|first row
        handles <scm|(cube 0)>|<point|4.28529898134674|0.74273713454161>>>>>>>|The
        Schema of Process Evolution in Evaluating <scm|(sum-cube 1 5)> >
      </float>, considering this in term of procedure <scm|sum-cube>, we take
      the the current value of <scm|a>, evaluate its cube, then add that
      intermediate value to the current value of <scm|result>, and that
      becomes the next entry in the <scm|result> column. Similarly, we can
      easily capture the rule for getting the next value for
      <scm|a><emdash>just increase <scm|a> by 1.

      And how do we know when we are ready to stop? Well, that's easy. The
      last row we want in the table is the one when <scm|a> is bigger than
      <scm|b>.

      Having done the analysis above, we are now able to write down our
      <scm|sum-cubes> procedure in a iterative way:

      <\scm-code>
        (define (sum-cubes cube a inc b)

        \ \ (define (iter a result)

        \ \ \ \ (if (\<gtr\> a b)

        \ \ \ \ \ \ \ \ result

        \ \ \ \ \ \ \ \ (iter (inc a) (+ (cube a) result))))

        \ \ (iter a 0))
      </scm-code>

      By following its prototype in the problem and generalizing our
      <scm|sum-cubes> above, we can easily complete the <scm|sum> procedure:

      <\scm-code>
        (define (sum term a next b)

        \ \ (define (iter a result)

        \ \ \ \ (if (\<gtr\> a b)

        \ \ \ \ \ \ \ \ result

        \ \ \ \ \ \ \ \ (iter (next a) (+ (term a) result))))

        \ \ (iter a 0))
      </scm-code>
    </answer>
  </render-exercise>
</body>

<\references>
  <\collection>
    <associate|Exercise_1.30-Table_1|<tuple|1|?>>
    <associate|Exercise_1.30-Table_2|<tuple|2|?>>
    <associate|Exercise_1.30-Table_3|<tuple|3|?>>
    <associate|Table_Exercise_1.30|<tuple|1|?>>
    <associate|Table_Exercise_1.30-1|<tuple|1|?>>
    <associate|Table_Exercise_1.30-2|<tuple|2|?>>
    <associate|auto-1|<tuple|1|?>>
    <associate|auto-2|<tuple|2|?>>
    <associate|auto-3|<tuple|3|?>>
    <associate|footnote-|<tuple|?|?>>
    <associate|footnote-*|<tuple|?|?>>
  </collection>
</references>

<\auxiliary>
  <\collection>
    <\associate|table>
      <tuple|normal||<pageref|auto-1>>

      <tuple|normal||<pageref|auto-2>>

      <tuple|normal||<pageref|auto-3>>
    </associate>
  </collection>
</auxiliary>