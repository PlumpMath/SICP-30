<TeXmacs|1.0.7.19>

<style|generic>

<\body>
  <\render-exercise|Exercise 2.6>
    <render-footnote|*|Creative Commons <image|<tuple|<#>|png>|1.20cm|0.42cm||>
    2013, Lawrence R. Amlord(<with|font|heiti|\<#989C\>\<#4E16\>\<#654F\>>).>

    In case representing pairs as procedures wasn't mind-boggling enough,
    consider that, in a language that can manipulate procedures, we can get
    by without numbers (at least insofar as nonnegative integers are
    concerned) by implementing 0 and the operation of adding 1 as

    <\scm-code>
      (define zero (lambda (f) (lambda (x) x)))

      \;

      (define (add-1 n)

      \ \ (lambda (f) (lambda (x) (f ((n f) x)))))\ 
    </scm-code>

    This representation is known as <em|Church numerals>, after its inventor,
    Alonzo Church, the logician who invented the <math|\<mathlambda\>>
    calculus.

    Define <scm|one> and <scm|two> directly (not in terms of <scm|zero> and
    <scm|add-1>). (Hint: Use substitution to evaluate <scm|(add-1 zero)>).
    Give a direct definition of the addition procedure <scm|+> (not in terms
    of repeated application of <scm|add-1>).

    <\answer>
      \;

      As is indicated in the problem discription, to represent <scm|one> here
      in the Church numeral, just evaluate <scm|(add-1 zero)> by means of
      substitution:

      <\scm-code>
        (add-1 zero)

        (lambda (f)

        \ \ (lambda (x)

        \ \ \ \ (f ((zero f) x))))

        (lambda (f)

        \ \ (lambda (x)

        \ \ \ \ (f (((lambda (f)

        \ \ \ \ \ \ \ \ \ \ \ (lambda (x) x))

        \ \ \ \ \ \ \ \ \ f)

        \ \ \ \ \ \ \ \ x))))

        (lambda (f)

        \ \ (lambda (x)

        \ \ \ \ (f ((lambda (x) x) x))))

        (lambda (f)

        \ \ (lambda (x)

        \ \ \ \ (f x)))
      </scm-code>

      Therefore, <scm|one> can be expressed in terms of procedure as:

      <\scm-code>
        (define one

        \ \ (lambda (f)

        \ \ \ \ (lambda (x)

        \ \ \ \ \ \ (f x))))
      </scm-code>

      Further more, the procedural definition of <scm|two> can also be
      obtained by evaluating <scm|(add-1 one)>:\ 

      <\scm-code>
        (define two

        \ \ (lambda (f)

        \ \ \ \ (lambda (x)

        \ \ \ \ \ \ (f (f x)))))
      </scm-code>

      Now, let's take up to bring the definition of the addition procedure
      <scm|+> to the surface. Since the addition operation <scm|+> is
      performed on integers, we'd better clarify a question at first: what is
      the general representation of an integer here in the Church numerals?

      Well, by investigating the shapes of procedural definition of
      <scm|zero>, <scm|one> and <scm|two>, we see that the successor of an
      integer is obtained by wrapping an additional <scm|f> around the
      integer itself. Hence, an arbitrary integer <math|n> can be represented
      by a procedure in which we successively applying <scm|f> to <scm|x> for
      <math|n> times: <math|f <around*|(|f <around*|(|\<ldots\><around*|(|f
      <around*|(|x|)>|)>\<ldots\>|)>|)>>. But, it is unacceptable to
      implement this idea in the program by embracing <scm|f> to its former
      result over and over again.

      On the other hand, while following the process generated in evaluating
      <scm|zero> and <scm|add-1>, we find that <scm|zero> is defined to be a
      procedure which takes as its argument a procedure and returns as its
      value a procedure. The procedure generated in evaluating <scm|(zero f)>
      takes in an object and returns that object invariantly. By comparision,
      the procedure <scm|add-1> is established on a relatively higher level
      where it manipulates procedurally defined numerals like <scm|zero>.
      Whenever <scm|(add-1 n)> is evaluated, the underlying expression
      <scm|((n f) x)> generates a procedure to represent the number <math|n>
      in our sence. This inspires us to represent the number <math|n>
      procedurally as:

      <\scm-code>
        (define n\ 

        \ \ (lambda (f)

        \ \ \ \ (lambda (x)

        \ \ \ \ \ \ ((n f) x))))
      </scm-code>

      Now, let's try to express the idea of addition by Church numerals. The
      former analysis on how <scm|zero> and <scm|(add-1 n)> work has given us
      some intuition in doing this. We saw that an arbitrary nonnegtive
      integer <math|n> is expressed by repeatedly applying <scm|f> to <scm|x>
      for <math|n> times. On the other hand, we can express <math|n>
      mathematically as <math|n=n+0>. So in any case, the expression <scm|((n
      f) x)> generates a procedure to represent the number <math|n> as well
      as adding <math|n> to <math|0>.

      Suppose that we want to add up two nonnegtive integers, say, <math|m>
      and <math|n> in terms of Church numerals. Using the evaluation rule
      above we've just discovered, the operation adding <math|m> to <math|n>
      should be expressed by a procedure which repeatedly applise <scm|f> to
      <scm|n> for <math|m> times. This reveals the definition of the addition
      procedure <scm|+>:

      <\scm-code>
        (define (+ m n)

        \ \ (lambda (f)

        \ \ \ \ (lambda (x)

        \ \ \ \ \ \ ((m f) ((n f) x)))))
      </scm-code>
    </answer>
  </render-exercise>
</body>

<\initial>
  <\collection>
    <associate|font-base-size|11>
  </collection>
</initial>

<\references>
  <\collection>
    <associate|footnote-|<tuple|?|?>>
    <associate|footnote-*|<tuple|?|1>>
  </collection>
</references>