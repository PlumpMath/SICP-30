<TeXmacs|1.0.7.20>

<style|generic>

<\body>
  <\render-exercise|Exercise 4.17>
    <render-footnote|*|Creative Commons <image|<tuple|<#>|png>|1.20cm|0.42cm||>
    2014, Lawrence X. Amlord (<with|font|heiti|\<#989C\>\<#4E16\>\<#654F\>>,
    aka <with|font|heiti|\<#989C\> \<#5E8F\>>).<new-line>
    <hspace|2.20ex>Email address: informlarry@gmail.com>

    Draw diagrams of the environment in effect when evaluating the expression
    <math|\<less\>e3\<gtr\>> in the procedure in the text, comparing how this
    will be structured when definitions are interpreted sequentially with how
    it will be structured if definitions are scanned out as described. Why is
    there an extra frame in the transformed program? Explain why this
    difference in environment structure can never make a difference in the
    behavior of a correct program. Design a way to make the interpreter
    implement the ``simultaneous'' scope rule for internal definitions
    without constructing the extra frame.

    <\answer>
      \;

      Figure <reference|Exercise_4.17-Figure_1><\float|float|tbh>
        <big-figure|<label|Exercise_4.17-Figure_1><with|gr-mode|<tuple|group-edit|group-ungroup>|gr-frame|<tuple|scale|1cm|<tuple|0.509999gw|0.5gh>>|gr-geometry|<tuple|geometry|1par|0.6par>|gr-grid|<tuple|empty>|gr-grid-old|<tuple|cartesian|<point|0|0>|1>|gr-edit-grid-aspect|<tuple|<tuple|axes|none>|<tuple|1|none>|<tuple|10|none>>|gr-edit-grid|<tuple|empty>|gr-edit-grid-old|<tuple|cartesian|<point|0|0>|1>|gr-arrow-end|\<gtr\>|gr-auto-crop|true|<graphics||<text-at|<scm|parameters:
        \<less\>vars\<gtr\>>|<point|2.18992078581823|-1.26907027358116>>|<point|3.3|8.32667268468867e-17>|<with|arrow-end|\<gtr\>|<line|<point|3.3|8.32667e-17>|<point|4.0|0.0>|<point|4.0|1.0>>>|<carc|<point|3.0|8.32667268468867e-17>|<point|3.6|8.32667268468867e-17>|<point|3.3|-0.3>>|<point|2.7|8.32667268468867e-17>|<carc|<point|2.4|8.32667268468867e-17>|<point|3.0|8.32667268468867e-17>|<point|2.7|-0.3>>|<with|arrow-end|\<gtr\>|<line|<point|2.7|8.32667268468867e-17>|<point|2.7|-1.0>>>|<text-at|<scm|body:
        (define u \<less\><with|prog-font-shape|italic|e1>\<gtr\>)>|<point|2.2|-1.7>>|<text-at|<scm|(define
        v \<less\><with|prog-font-shape|italic|e2>\<gtr\>)>|<point|3.2|-2.1>>|<text-at|<scm|\<less\><with|prog-font-shape|italic|e3>\<gtr\>)>|<point|3.2|-2.45799606469946>>|<with|arrow-end|\<gtr\>|<line|<point|-3.82432|1.96526>|<point|3.0|2.0>|<point|3.0|0.3>>>|<text-at|<scm|proc:>|<point|-4.76624|1.93221091659999>>|<text-at|<scm|\<less\>vars\<gtr\>:
        \<less\>args\<gtr\>>|<point|-4.1904456392829|-0.3>>|<with|arrow-end|\<gtr\>|<line|<point|-5.8|2.0>|<point|-5.0|2.0>>>|<with|arrow-end|\<gtr\>|<line|<point|-3.00997|0.0999802>|<point|-3.0|1.0>>>|<gr-group|<text-at|<scm|env>|<point|-6.9|1.7>>|<text-at|<scm|global>|<point|-6.9|2.1>>>|<cline|<point|-5.0|3.0>|<point|5.0|3.0>|<point|5.0|1.0>|<point|-5.0|1.0>>|<text-at|<scm|u:
        \<less\><with|prog-font-shape|italic|e1>\<gtr\>>|<point|-4.1904456392829|-0.7>>|<text-at|<scm|v:
        \<less\><with|prog-font-shape|italic|e2>\<gtr\>>|<point|-4.1904456392829|-1.1>>|<text-at|<scm|\<less\><with|prog-font-shape|italic|e3>\<gtr\>>|<point|-3.3904456392829|-1.65799606469946>>|<with|arrow-end|\<gtr\>|<line|<point|-4.97998|-0.604359>|<point|-4.38696174361688|-0.618216653128721>>>|<text-at|<scm|E1>|<point|-5.39045|-0.7>>|<cline|<point|-4.3904456392829|0.1>|<point|-1.6904456392829|0.1>|<point|-1.6904456392829|-1.3>|<point|-4.3904456392829|-1.3>>>>|Environments
        created by evaluating <math|\<less\>e3\<gtr\>>where definitions are
        interpreted sequentially.>
      </float> and figure <reference|Exercise_4.17-Figure_2><\float|float|tbh>
        <big-figure|<label|Exercise_4.17-Figure_2><with|gr-mode|<tuple|group-edit|move>|gr-frame|<tuple|scale|1cm|<tuple|0.509999gw|0.559994gh>>|gr-geometry|<tuple|geometry|1par|0.6par>|gr-grid|<tuple|empty>|gr-grid-old|<tuple|cartesian|<point|0|0>|1>|gr-edit-grid-aspect|<tuple|<tuple|axes|none>|<tuple|1|none>|<tuple|10|none>>|gr-edit-grid|<tuple|empty>|gr-edit-grid-old|<tuple|cartesian|<point|0|0>|1>|gr-auto-crop|true|<graphics||<text-at|<scm|parameters:
        \<less\>vars\<gtr\>>|<point|1.68992078581823|-1.26907027358116>>|<text-at|<scm|(set!
        v \<less\><with|prog-font-shape|italic|><with|prog-font-shape|italic|e2>\<gtr\>)>|<point|3.0820198902755|-2.7>>|<carc|<point|2.5|-8.32667268468867e-17>|<point|3.1|-8.32667268468867e-17>|<point|2.8|-0.3>>|<point|2.8|-8.32667268468867e-17>|<text-at|<scm|\<less\><with|prog-font-shape|italic|e3>\<gtr\>)>|<point|3.0609898902755|-3.06919289432163>>|<text-at|<scm|(v
        '*unassigned*))>|<point|3.71011119193743|-2.03502176517848>>|<with|arrow-end|\<gtr\>|<line|<point|2.2|-8.32667268468867e-17>|<point|2.2|-1.0>>>|<text-at|<scm|body:
        (let ((u '*unassigned*)>|<point|1.7|-1.7>>|<text-at|<scm|proc:>|<point|-4.76624|1.93221091659999>>|<text-at|<scm|\<less\>vars\<gtr\>:
        \<less\>args\<gtr\>>|<point|-4.1301756392829|-0.6>>|<with|arrow-end|\<gtr\>|<line|<point|-5.8|2.0>|<point|-5.0|2.0>>>|<gr-group|<text-at|<scm|env>|<point|-6.9|1.7>>|<text-at|<scm|global>|<point|-6.9|2.1>>>|<cline|<point|-5.0|3.0>|<point|5.0|3.0>|<point|5.0|1.0>|<point|-5.0|1.0>>|<text-at|<scm|\<less\><with|prog-font-shape|italic|e3>\<gtr\>>|<point|-3.2904456392829|-3.13726606469946>>|<text-at|<scm|E2>|<point|-5.39045|-2.3>>|<text-at|<scm|v:
        \<less\><with|prog-font-shape|italic|e2>\<gtr\>>|<point|-3.5904456392829|-2.47927>>|<with|arrow-end|\<gtr\>|<line|<point|-5.0|-2.2>|<point|-4.4|-2.2>>>|<carc|<point|1.9|-8.32667268468867e-17>|<point|2.5|-8.32667268468867e-17>|<point|2.2|-0.3>>|<with|arrow-end|\<gtr\>|<line|<point|2.8|-8.32667268468867e-17>|<point|3.5|-8.32667268468867e-17>|<point|3.5|1.0>>>|<text-at|<scm|(set!
        u \<less\><with|prog-font-shape|italic|e1>\<gtr\>)>|<point|3.08201491573543|-2.4>>|<cline|<point|-4.4|0>|<point|-1.6|0.0>|<point|-1.6|-1.0>|<point|-4.4|-1.0>>|<text-at|<scm|u:
        \<less\><with|prog-font-shape|italic|e1>\<gtr\>>|<point|-3.5904456392829|-2.07927>>|<with|arrow-end|\<gtr\>|<line|<point|-3.0|0.0>|<point|-3.0|1.0>>>|<with|arrow-begin|\<less\>|<line|<point|-3|-1>|<point|-3.0|-1.6>>>|<point|2.2|-8.32667268468867e-17>|<with|arrow-end|\<gtr\>|<line|<point|-3.82431538563302|1.96527318428364>|<point|2.5|2.0>|<point|2.5|0.3>>>|<cline|<point|-4.4|-1.6>|<point|-1.6|-1.6>|<point|-1.6|-2.8>|<point|-4.4|-2.8>>>>|Environments
        created by evaluating <math|\<less\>e3\<gtr\>>where definitions are
        scanned out.>
      </float> each shows the point in the evaluation of
      <math|\<less\>e3\<gtr\>> in the procedure in the text where definitions
      are either interpreted sequentially or scanned out as described. Recall
      from section 1.3.2 that <scm|let> is simply syntactic sugar for a
      procedure call:

      <\scm-code>
        (let ((u '*unassigned*)

        \ \ \ \ \ \ (v '*unassigned*))

        \ \ (set! u \<less\>e1\<gtr\>)

        \ \ (set! v \<less\>e2\<gtr\>)

        \ \ \<less\>e3\<gtr\>)
      </scm-code>

      is interpreted as an alternate syntax for

      <\scm-code>
        ((lambda (u v)

        \ \ \ (set! u \<less\>e1\<gtr\>)

        \ \ \ (set! v \<less\>e2\<gtr\>)

        \ \ \ \<less\>e3\<gtr\>)

        \ '*unassigned* '*unassigned*)
      </scm-code>

      The latter one is a procedure application and by the environment model
      of evaluation, we extend the base environment to include a frame that
      binds the parameters of the procedure (<scm|u> and <scm|v>) to the
      arguments to which the procedure is to be applied (<scm|'*unassigned*>
      and <scm|'*unassigned*>). Hence, the diagram of transformed program
      includes an extra frame.<new-line> <hspace|2ex>Since our syntax
      transformation creates a <scm|let> to embrace all the internal
      definitions in the body of the <scm|lambda> expression. This doesn't
      extend or shrink the scope of any variable inside the procedure. Hence
      this difference in environment structure can never make a difference in
      the behavior of a correct program.<new-line> <hspace|2ex>Alternatively,
      there is a strategy in which the interpreter implement the
      ``simultaneous'' scope rule for internal definitions without
      constructing the extra frame. To do this, we rearrange the sequence of
      expressions and make internal definitions come first inside the
      <scm|lambda> expression. Note that the body of a <scm|lambda>
      expression is evaluated only when the procedure is applied to
      arguments. So all the internal definitions can use variables that
      defined after it. This automatically brings us the ``simultaneous''
      scope for internal definitions and no extra frame is created in the
      process.
    </answer>
  </render-exercise>
</body>

<\initial>
  <\collection>
    <associate|font-base-size|11>
  </collection>
</initial>

<\references>
  <\collection>
    <associate|Exercise_4.17-Figure_1|<tuple|1|1>>
    <associate|Exercise_4.17-Figure_2|<tuple|2|1>>
    <associate|auto-1|<tuple|1|1>>
    <associate|auto-2|<tuple|2|1>>
    <associate|footnote-|<tuple|?|?|../../../../../../.TeXmacs/texts/scratch/no_name_23.tm>>
    <associate|footnote-*|<tuple|?|1>>
  </collection>
</references>

<\auxiliary>
  <\collection>
    <\associate|figure>
      <tuple|normal|Environments created by evaluating
      <with|mode|<quote|math>|\<less\>e3\<gtr\>>where definitions are
      interpreted sequentially.|<pageref|auto-1>>

      <tuple|normal|Environments created by evaluating
      <with|mode|<quote|math>|\<less\>e3\<gtr\>>where definitions are scanned
      out.|<pageref|auto-2>>
    </associate>
  </collection>
</auxiliary>