<TeXmacs|1.0.7.20>

<style|generic>

<\body>
  <\render-exercise|Exercise 4.20>
    <render-footnote|*|Creative Commons <image|<tuple|<#>|png>|1.20cm|0.42cm||>
    2014, Lawrence X. Amlord (<with|font|heiti|\<#989C\>\<#4E16\>\<#654F\>>,
    aka <with|font|heiti|\<#989C\> \<#5E8F\>>).<new-line>
    <hspace|2.20ex>Email address: informlarry@gmail.com>

    Because internal definitions look sequential but are actually
    simultaneous, some people prefer to avoid them entirely, and use the
    special form <scm|letrec> instead. <scm|Letrec> looks like <scm|let>, so
    it is not surprising that the variables it binds are bound simultaneously
    and have the same scope as each other. The sample procedure <scm|f> above
    can be written without internal definitions, but with exactly the same
    meaning, as

    <\scm-code>
      (define (f x)

      \ \ (letrec ((even?

      \ \ \ \ \ \ \ \ \ \ \ \ (lambda (n)

      \ \ \ \ \ \ \ \ \ \ \ \ \ \ (if (= n 0)

      \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ true

      \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ (odd? (- n 1)))))

      \ \ \ \ \ \ \ \ \ \ \ (odd?

      \ \ \ \ \ \ \ \ \ \ \ \ (lambda (n)

      \ \ \ \ \ \ \ \ \ \ \ \ \ \ (if (= n 0)

      \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ false

      \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ (even? (- n 1))))))

      \ \ \ \ \<less\><with|prog-font-shape|italic|rest of body of>
      f\<gtr\>))
    </scm-code>

    <scm|Letrec> expression, which have the form

    <\scm-code>
      (letrec ((\<less\><math|var<rsub|1>>\<gtr\>
      \<less\><math|exp<rsub|1>>\<gtr\>) ...
      (\<less\><math|var<rsub|n>>\<gtr\> \<less\><math|exp<rsub|n>>\<gtr\>))

      \ \ \<less\>body\<gtr\>)
    </scm-code>

    are a variation on <scm|let> in which the expressions
    <math|\<less\>exp<rsub|k>\<gtr\>> that provide the initial values for the
    variables <math|\<less\>var<rsub|k>\<gtr\>> are evaluated in an
    environment that includes all the <scm|letrec> bindings. This permits
    recursion in the bindings, such as the mutual recursion of <scm|even?>
    and <scm|odd?> in the example above, or the evaluation of <scm|10>
    factorial with

    <\scm-code>
      (letrec ((fact

      \ \ \ \ \ \ \ \ \ \ (lambda (n)

      \ \ \ \ \ \ \ \ \ \ \ \ (if (= n 1)

      \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ 1

      \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ (* n (fact (- n 1)))))))

      \ \ (fact 10))
    </scm-code>

    a. Implement <scm|letrec> as a derived expression, by transforming a
    <scm|letrec> expression into a <scm|let> expression as shown in the text
    above or in exercise 4.18. That is, the <scm|letrec> variables should be
    created with a <scm|let> and then be assigned their values with
    <scm|set!>.

    b. Louis Reasoner is confused by all this fuss about internal
    definitions. The way he sees it, if you don't like to use <scm|define>
    inside a procedure, you can just use <scm|let>. Illustrate what is loose
    about his reasoning by drawing an environment diagram that shows the
    environment in which the <scm|\<less\><with|prog-font-shape|italic|rest
    of body of> f\<gtr\>> is evaluated during evaluation of the expression
    <scm|(f 5)>, with <scm|f> defined as in this exercise. Draw an
    environment diagram for the same evaluation, but with <scm|let> in place
    of <scm|letrec> in the definition of <scm|f>.

    <\answer>
      \;

      a. To implement <scm|letrec> as a derived expression, we include syntax
      procedures that extract parts of a <scm|letrec> expression, and a
      procedure <scm|letrec-\<gtr\>let> that transforms <scm|letrec>
      expressions into <scm|let> expressions.

      <\scm-code>
        (define (letrec? exp) (tagged-list? exp 'letrec))

        \;

        (define (letrec-bindings exp) (caddr exp))

        \;

        (define (letrec-body exp) (cddr exp))

        \;

        (define (binding-variable binding) (car binding))

        \;

        (define (letrec-\<gtr\>let exp)

        \ \ (let ((bindings (letrec-bindings exp))

        \ \ \ \ \ \ \ \ (body (letrec-body exp)))

        \ \ \ \ (let ((vars (filter binding-variable bindings)))

        \ \ \ \ \ \ (let ((inits (map (lambda (var) (cons var '*unassigned*))

        \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ vars))

        \ \ \ \ \ \ \ \ \ \ \ \ (assignments (map (lambda (binding) (cons
        'set! binding))

        \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ bindings)))

        \ \ \ \ \ \ \ \ (cons 'let

        \ \ \ \ \ \ \ \ \ \ \ \ \ \ (cons inits

        \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ (append assignments
        body)))))))
      </scm-code>

      b. To draw out the environment diagram in evaluating
      <scm|\<less\><with|prog-font-shape|italic|rest body of> f\<gtr\>> in
      <scm|(f 5)>, we first substitute the <scm|letrec> expression of <scm|f>
      with an equivalent <scm|let> expression.

      <\scm-code>
        (define (f x)

        \ \ (let ((even? '*unassigned*)

        \ \ \ \ \ \ \ \ (odd? '*unassigned*))

        \ \ \ \ (set! even?

        \ \ \ \ \ \ \ \ \ \ (lambda (n)

        \ \ \ \ \ \ \ \ \ \ \ \ (if (= n 0)

        \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ true

        \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ (odd? (- n 1)))))

        \ \ \ \ (set! odd?

        \ \ \ \ \ \ \ \ \ \ (lambda (n)

        \ \ \ \ \ \ \ \ \ \ \ \ (if (= n 0)

        \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ false

        \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ (even? (- n 1)))))

        \ \ \ \ \<less\><with|prog-font-shape|italic|rest of body of>
        f\<gtr\>))
      </scm-code>

      As shown in figure <reference|Exercise_4.20-Figure_1><\float|float|tbh>
        <big-figure|<label|Exercise_4.20-Figure_1><with|gr-mode|<tuple|group-edit|move>|gr-frame|<tuple|scale|1cm|<tuple|0.569993gw|0.5gh>>|gr-geometry|<tuple|geometry|1par|0.6par>|gr-grid|<tuple|empty>|gr-grid-old|<tuple|cartesian|<point|0|0>|1>|gr-edit-grid-aspect|<tuple|<tuple|axes|none>|<tuple|1|none>|<tuple|10|none>>|gr-edit-grid|<tuple|empty>|gr-edit-grid-old|<tuple|cartesian|<point|0|0>|1>|gr-arrow-end|\<gtr\>|gr-auto-crop|true|<graphics||<cline|<point|-6.0|3.0>|<point|6.0|3.0>|<point|6.0|1.0>|<point|-6.0|1.0>>|<text-at|<scm|env>|<point|-7.9|1.8>>|<text-at|<scm|global>|<point|-7.9|2.1>>|<with|arrow-end|\<gtr\>|<line|<point|-6.8|2>|<point|-6.0|2.0>>>|<text-at|<scm|f:>|<point|-5.8|1.9>>|<point|-4.2|2.77555756156289e-17>|<carc|<point|-4.5|2.77555756156289e-17>|<point|-3.9|2.77555756156289e-17>|<point|-4.2|0.3>>|<carc|<point|-5.1|2.77555756156289e-17>|<point|-4.5|2.77555756156289e-17>|<point|-4.8|0.3>>|<point|-4.8|2.77555756156289e-17>|<with|arrow-end|\<gtr\>|<line|<point|-5.35798|1.99697>|<point|-4.5|2.0>|<point|-4.5|0.3>>>|<text-at|<scm|\<less\><with|prog-font-shape|italic|rest
        of body of> f\<gtr\>)>|<point|-5.7|-2.4>>|<text-at|<scm|(odd?
        ...))>|<point|-4.54686589119784|-2.0876141023945>>|<text-at|<scm|parameters:
        x>|<point|-7.0|-1.3>>|<text-at|<scm|body: (letrec ((even?
        ...)>|<point|-7.0|-1.7>>|<with|arrow-end|\<gtr\>|<line|<point|-4.8|2.77556e-17>|<point|-6.0|0.0>|<point|-6.0|-1.0>>>|<with|arrow-end|\<gtr\>|<line|<point|-4.2|2.77556e-17>|<point|-3.0|0.0>|<point|-3.0|1.0>>>|<gr-group|<cline|<point|0.3|0.5>|<point|1.7|0.5>|<point|1.7|-0.1>|<point|0.3|-0.1>>|<text-at|<scm|x:
        5>|<point|0.7|0.1>>>|<text-at|<scm|even?:
        '*unassigned*>|<point|-0.699999999999999|-1.1>>|<text-at|<scm|odd?:
        '*unassigned*>|<point|-0.699999999999999|-1.5>>|<cline|<point|-1.0|-0.6>|<point|3.0|-0.6>|<point|3.0|-1.8>|<point|-1.0|-1.8>>|<with|arrow-begin|\<less\>|<line|<point|1|1>|<point|0.997403757110729|0.485298981346739>>>|<with|arrow-begin|\<less\>|<line|<point|0.997404|-0.107372>|<point|0.997403757110729|-0.615375711072893>>>|<text-at|<scm|E1>|<point|-1.9508600887849|-1.31755>>|<with|arrow-end|\<gtr\>|<line|<point|-1.536674|-1.17927>|<point|-1.036674|-1.17927>>>|<text-at|<scm|(set!
        even? ...)>|<point|-0.6|-2.2>>|<text-at|<scm|(set! odd?
        ...)>|<point|-0.6|-2.6>>|<text-at|<scm|\<less\><with|prog-font-shape|italic|rest
        of body of> f\<gtr\>>|<point|-0.6|-2.9>>>>|Environment structure
        produced by evaluating <scm|\<less\>rest of body of f\<gtr\>> in
        <scm|(f 5)> using <scm|letrec>.>
      </float>, we can see that both <scm|even?> and <scm|odd?> reside in the
      same environment, that is, they have simultaneous scope. Therefore,
      mutual recursion can be dealt correctly and the evaluator would behave
      as what we originally expected.<new-line> <hspace|2ex>However,
      following Louis's illusion, which simply use <scm|let> in place of
      <scm|letrec>, the behavior of the interpreter in evaluating
      <scm|\<less\><with|prog-font-shape|italic|rest of body of> f\<gtr\>>
      varies a lot. As figure <reference|Exercise_4.20-Figure_2><\float|float|tbh>
        <big-figure|<label|Exercise_4.20-Figure_2><with|gr-mode|<tuple|edit|line>|gr-frame|<tuple|scale|1cm|<tuple|0.569993gw|0.629988gh>>|gr-geometry|<tuple|geometry|1par|0.6par>|gr-grid|<tuple|empty>|gr-grid-old|<tuple|cartesian|<point|0|0>|1>|gr-edit-grid-aspect|<tuple|<tuple|axes|none>|<tuple|1|none>|<tuple|10|none>>|gr-edit-grid|<tuple|empty>|gr-edit-grid-old|<tuple|cartesian|<point|0|0>|1>|gr-arrow-end|\<gtr\>|gr-auto-crop|true|<graphics||<cline|<point|-6.0|3.0>|<point|6.0|3.0>|<point|6.0|1.0>|<point|-6.0|1.0>>|<text-at|<scm|env>|<point|-7.9|1.8>>|<text-at|<scm|global>|<point|-7.9|2.1>>|<with|arrow-end|\<gtr\>|<line|<point|-6.8|2>|<point|-6.0|2.0>>>|<text-at|<scm|f:>|<point|-5.8|1.9>>|<point|-4.2|2.77555756156289e-17>|<carc|<point|-4.5|2.77555756156289e-17>|<point|-3.9|2.77555756156289e-17>|<point|-4.2|0.3>>|<carc|<point|-5.1|2.77555756156289e-17>|<point|-4.5|2.77555756156289e-17>|<point|-4.8|0.3>>|<point|-4.8|2.77555756156289e-17>|<with|arrow-end|\<gtr\>|<line|<point|-5.35798|1.99697>|<point|-4.5|2.0>|<point|-4.5|0.3>>>|<text-at|<scm|\<less\><with|prog-font-shape|italic|rest
        of body of> f\<gtr\>)>|<point|-5.7|-2.4>>|<text-at|<scm|(odd?
        ...))>|<point|-5.04686589119784|-2.0876141023945>>|<text-at|<scm|parameters:
        x>|<point|-7.0|-1.3>>|<text-at|<scm|body: (let ((even?
        ...)>|<point|-7.0|-1.7>>|<with|arrow-end|\<gtr\>|<line|<point|-4.8|2.77556e-17>|<point|-6.0|0.0>|<point|-6.0|-1.0>>>|<with|arrow-end|\<gtr\>|<line|<point|-4.2|2.77556e-17>|<point|-3.0|0.0>|<point|-3.0|1.0>>>|<gr-group|<cline|<point|0.3|0.5>|<point|1.7|0.5>|<point|1.7|-0.1>|<point|0.3|-0.1>>|<text-at|<scm|x:
        5>|<point|0.7|0.1>>>|<text-at|<scm|even?:
        >|<point|-0.699999999999999|-1.1>>|<text-at|<scm|odd?:
        >|<point|-0.699999999999999|-1.5>>|<cline|<point|-1.0|-0.6>|<point|3.0|-0.6>|<point|3.0|-1.8>|<point|-1.0|-1.8>>|<with|arrow-begin|\<less\>|<line|<point|1|1>|<point|0.997403757110729|0.485298981346739>>>|<with|arrow-begin|\<less\>|<line|<point|0.997404|-0.107372>|<point|0.997403757110729|-0.615375711072893>>>|<text-at|<scm|E1>|<point|-1.9508600887849|-1.31755>>|<with|arrow-end|\<gtr\>|<line|<point|-1.536674|-1.17927>|<point|-1.036674|-1.17927>>>|<text-at|<scm|\<less\><with|prog-font-shape|italic|rest
        of body of> f\<gtr\>>|<point|-0.6|-2.5>>|<text-at|<scm|body: (if (= n
        0) ...)>|<point|-3.5|-4.9>>|<text-at|<scm|parameters:
        x>|<point|-3.5|-4.5>>|<point|-1.7|-3.5>|<carc|<point|-2.0|-3.5>|<point|-1.4|-3.5>|<point|-1.7|-3.2>>|<point|-2.3|-3.5>|<carc|<point|-2.6|-3.5>|<point|-2.0|-3.5>|<point|-2.3|-3.2>>|<point|4.3|-3.5>|<carc|<point|4.0|-3.5>|<point|4.6|-3.5>|<point|4.3|-3.2>>|<point|3.7|-3.5>|<carc|<point|3.4|-3.5>|<point|4.0|-3.5>|<point|3.7|-3.2>>|<text-at|<scm|parameters:
        x>|<point|2.42520174626273|-4.52919036909644>>|<text-at|<scm|body:
        (if (= n 0) ...)>|<point|2.42520174626273|-4.92919036909644>>|<with|arrow-end|\<gtr\>|<line|<point|0.408569|-1.40301>|<point|1.0|-1.4>|<point|1.0|-1.6>|<point|-2.0|-1.6>|<point|-2.0|-3.2>>>|<with|arrow-end|\<gtr\>|<line|<point|0.383565947876703|-1.00112448736605>|<point|4.0|-1.0>|<point|4.0|-3.2>>>|<with|arrow-end|\<gtr\>|<line|<point|3.7|-3.5>|<point|3.7|-4.24404683159148>>>|<with|arrow-end|\<gtr\>|<line|<point|-2.3|-3.5>|<point|-2.2834534991401|-4.21848128059267>>>|<with|arrow-end|\<gtr\>|<line|<point|4.3|-3.5>|<point|5.0|-3.5>|<point|5.0|0.2>|<point|1.69590885037703|0.192799973541474>>>|<with|arrow-end|\<gtr\>|<line|<point|-0.22989403743506|0.34992>|<point|0.27010596256494|0.34992>>>|<text-at|<scm|E2>|<point|-0.64408012621996|0.21164>>|<with|arrow-end|\<gtr\>|<line|<point|-1.7|-3.5>|<point|-0.8|-3.5>|<point|-0.8|0.0>|<point|0.298898663844424|-0.00825175287736473>>>>>|Environment
        structure produced by evaluating <scm|\<less\>rest of body of
        f\<gtr\>> in <scm|(f 5)> using <scm|let>.>
      </float> shows, evaluating <scm|even?> and <scm|odd?> both create a
      procedure object whose environment are E2, rather than E1. This makes
      them invisible to each other when the interpreter comes to the
      evaluation of <scm|\<less\>rest of body of
      <with|prog-font-shape|italic|f>\<gtr\>>. Hence, by Louis's strategy,
      the evaluator will finally run into a sort of error like ``<scm|Unbound
      variable: even?>''.
    </answer>
  </render-exercise>
</body>

<\initial>
  <\collection>
    <associate|font-base-size|11>
  </collection>
</initial>

<\references>
  <\collection>
    <associate|Exercise_4.20-Figure_1|<tuple|1|2>>
    <associate|Exercise_4.20-Figure_2|<tuple|2|3>>
    <associate|auto-1|<tuple|1|2>>
    <associate|auto-2|<tuple|2|3>>
    <associate|footnote-|<tuple|?|?|../../../../../../.TeXmacs/texts/scratch/no_name_23.tm>>
    <associate|footnote-*|<tuple|?|1>>
  </collection>
</references>

<\auxiliary>
  <\collection>
    <\associate|figure>
      <tuple|normal|Environment structure produced by evaluating
      <with|mode|<quote|prog>|prog-language|<quote|scheme>|font-family|<quote|rm>|\<less\>rest
      of body of f\<gtr\>> in <with|mode|<quote|prog>|prog-language|<quote|scheme>|font-family|<quote|rm>|(f
      5)> using <with|mode|<quote|prog>|prog-language|<quote|scheme>|font-family|<quote|rm>|letrec>.|<pageref|auto-1>>

      <tuple|normal|Environment structure produced by evaluating
      <with|mode|<quote|prog>|prog-language|<quote|scheme>|font-family|<quote|rm>|\<less\>rest
      of body of f\<gtr\>> in <with|mode|<quote|prog>|prog-language|<quote|scheme>|font-family|<quote|rm>|(f
      5)> using <with|mode|<quote|prog>|prog-language|<quote|scheme>|font-family|<quote|rm>|let>.|<pageref|auto-2>>
    </associate>
  </collection>
</auxiliary>