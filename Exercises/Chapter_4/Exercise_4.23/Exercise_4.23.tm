...
<\body>
  <\render-exercise|Exercise 4.23>
    ...
    Alyssa P. Hacker doesn't understand why <scm|analyze-sequence> needs to
    be so complicated. All the other analysis procedures are straightforward
    transformations of the corresponding evaluation procedures (or <scm|eval>
    clauses) in section 4.1.1. She expected <scm|analyze-sequence> to look
    like this:
    ...
    sequence have been analyzed, the sequence itself has not been.<new-line>
    <hspace|2ex>Compare the two versions of <scm|analyze-sequence>. For
    example, consider the common case (typical of procedure bodies) where the
    sequence has just one expression. What work will the execution procedure
    ...
    <\answer>
      ...
      We compare the two version of <scm|analyze-sequence> by investigating
      the result they produce while evaluating the expression:

      <\scm-code>
        (analyze (\<less\>procedure\<gtr\> \<less\>args\<gtr\>))
      </scm-code>
      ...
      <\scm-code>
        ...
        \ \ \ (extend-environment \<less\>parameters\<gtr\>
      ...
      <\scm-code>
        ...
        \ \ (execute-sequence (cdr (begin \<less\>body-exp1\<gtr\>
