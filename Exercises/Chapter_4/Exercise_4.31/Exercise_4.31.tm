...
<\body>
  <\render-exercise|Exercise 4.31>
    ...
    lazy evaluation as an <with|font-shape|italic|upward-compatible
    ...
    to let the user control whether or not arguments are to be delayed. While
    we're at it, we may as well also give the user the choice between
    ...
    would define <scm|f> to be a procedure of four arguments, where the first
    ...
    behavior as ordinary Scheme, while adding the <scm|lazy-memo> declaration
    to each parameter of every compound procedure will produce the behavior
    of the lazy evaluator defined in this section. Design and implement the
    changes required to produce such an extension to Scheme. You will have to
    implement new syntax procedures to handle the new syntax for
    <scm|define>. You must also arrange for <scm|eval> or <scm|apply> to
    ...
    <\answer>
      ...
      <with|font-series|bold|Modifying the evaluator><new-line>Extending the
      evaluator upward-compatibly in this way requires additional syntax
      procedures to determine the <scm|lazy> and <scm|lazy-memo> declaration
      ...
      <hspace|3ex><scm|Eval> remains the same because the new evaluator
      involves both applicative and normal order evaluation and
      <scm|actual-value> is devised to handle both cases. The essentials in
      extending the evaluator upward-compatibly is in the handling of
      procedure applications in <scm|apply> compared to the lazy evaluator.
      For primitive procedures, we still evaluate all the arguments before
      applying the primitive; for compound procedures, we either evaluate or
      delay an argument before applying the procedure, according to the
      programmer's choice.
      ...
      <\scm-code>
        ...
        \ \ \ \ \ \ \ \ \ \ \ (list-of-accord-args (procedure-parameters
        procedure) \ \ \ <with|prog-font-shape|italic|; changed>
      ...
      The procedure <scm|list-of-accord-args> performs selective evaluation
      on procedure arguemnts at the programmer's preference.
      ...
      <hspace|3ex>Finally, we also change the prompt to indicate that this is
      the upward-compatible evaluator:
      ...
      <hspace|3ex>With these changes made, we can start the evaluator and
      test it. The successful evaluation of the <scm|try >expression
      discussed in section 4.2.1 indicates that the interpreter is preforming
      upward-compatible evaluation:
      ...
      <\scm-code>
        ...
        <with|prog-font-shape|italic|;;; UC-Eval input:>

        (define (try a b)

        \ \ (if (= a 0) 1 b))

        <with|prog-font-shape|italic|;;; UC-Eval value:>

        <with|prog-font-shape|italic|ok>
        ...
        <\with|prog-font-shape|italic>
          ...
          ; (RESTART 1) =\<gtr\> Return to read-eval-print level 1.
        </with>
        ...
        <with|prog-font-shape|italic|;Abort!>
      ...
      <with|font-series|bold|Representing thunks><new-line>The extended
      evaluator must manage to create thunks correspondingly to meet the
      programmer's request and force them later. For the ordinary delayed
      request, it follows the way in the text by handling thunks without
      memoization; for requests of both delayed and memoized, thunks are
      turned into evaluated ones when forced
