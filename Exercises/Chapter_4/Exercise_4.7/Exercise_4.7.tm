<TeXmacs|1.0.7.20>

<style|generic>

<\body>
  <\render-exercise|Exercise 4.7>
    <render-footnote|*|Creative Commons <image|<tuple|<#>|png>|1.20cm|0.42cm||>
    2014, Lawrence X. Amlord (<with|font|heiti|\<#989C\>\<#4E16\>\<#654F\>>,
    aka <with|font|heiti|\<#989C\> \<#5E8F\>>).<new-line>
    <hspace|2.20ex>Email address: informlarry@gmail.com>

    <scm|Let*> is similar to <scm|let>, except that the bindings of the
    <scm|let*> variables are performed sequentially from left to right, and
    each binding is made in an environment in which all of the preceding
    bindings are visible. For example

    <\scm-code>
      (let* ((x 3)

      \ \ \ \ \ \ \ (y (+ x 2))

      \ \ \ \ \ \ \ (z (+ x y 5)))

      \ \ (* x z))
    </scm-code>

    returns 39. Explain how a <scm|let*> expression can be rewritten as a set
    of nested <scm|let> expressions, and write a procedure
    <scm|let*-\<gtr\>nested-lets> that performs this transformation. If we
    have already implemented <scm|let> (exercise 4.6) and we want to extend
    the evaluator to handle <scm|let*>, is it sufficient to add a clause to
    <scm|eval> whose action is

    <\scm-code>
      (eval (let*-\<gtr\>nested-lets exp) env)
    </scm-code>

    or must we explicitly expand <scm|let*> in terms of non-derived
    expressions?

    <\answer>
      \;

      Typically, a <scm|let*> expression

      <\scm-code>
        (let* ((\<less\><math|var<rsub|1>>\<gtr\>
        \<less\><math|exp<rsub|1>>\<gtr\>)

        \ \ \ \ \ \ \ (\<less\><math|var<rsub|2>>\<gtr\>
        \<less\><math|exp<rsub|2>>\<gtr\>)

        \ \ \ \ \ \ \ ...

        \ \ \ \ \ \ \ (\<less\><math|var<rsub|n>>\<gtr\>
        \<less\><math|exp<rsub|n>>\<gtr\>))

        \ \ \<less\>body\<gtr\>)
      </scm-code>

      is equivalent to

      <\scm-code>
        (let ((\<less\><math|var<rsub|1>>\<gtr\>
        \<less\><math|exp<rsub|1>>\<gtr\>))

        \ \ (let ((\<less\><math|var<rsub|2>>\<gtr\>
        \<less\><math|exp<rsub|2>>\<gtr\>))

        \ \ \ \ ...

        \ \ \ \ \ \ (let ((\<less\><math|var<rsub|n>>\<gtr\>
        \<less\><math|exp<rsub|n>>\<gtr\>))

        \ \ \ \ \ \ \ \ \<less\>body\<gtr\>)...))
      </scm-code>

      <hspace|3ex>To transform a <scm|let*> expression into a set of nested
      <scm|let> expressions, we need to include syntax procedures that
      extract the parts of a <scm|let*> expression. A <scm|let*> expression
      begin with <scm|let*> and has a list of associations and a body.
      Besides, we also need to reconstruct <scm|let*> expressions when the
      list of association in the original <scm|let*> expression contains more
      than one binding.

      <\scm-code>
        (define (let*? exp) (tagged-list? exp 'let*))

        \;

        (define (list-of-associations exp) (cadr exp))

        \;

        (define (let*-body exp) (cddr exp))

        \;

        (define (make-let assocs body)

        \ \ (list 'let assocs body))

        \;

        (define (make-let* assocs body)

        \ \ (list 'let* assocs body))

        \;

        (define (let*-\<gtr\>nested-lets exp)

        \ \ (let ((assocs (list-of-associations exp))

        \ \ \ \ \ \ \ \ (body (let*-body exp)))

        \ \ \ \ (if (null? assocs)

        \ \ \ \ \ \ \ \ 'false

        \ \ \ \ \ \ \ \ (let ((first-assoc (car assocs))

        \ \ \ \ \ \ \ \ \ \ \ \ \ \ (rest-assocs (cdr assocs)))

        \ \ \ \ \ \ \ \ \ \ (if (null? rest-assocs)

        \ \ \ \ \ \ \ \ \ \ \ \ \ \ (make-let (list first-assoc) body)

        \ \ \ \ \ \ \ \ \ \ \ \ \ \ (make-let (list first-assoc)

        \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ (let*-\<gtr\>nested-lets
        (make-let* rest-assocs body))))))))
      </scm-code>

      <hspace|3ex>If we have already implemented <scm|let> (exercise 4.6) and
      we want to extend the evaluator to handle <scm|let*>. It is sufficient
      to add a clause to <scm|eval> whose action is

      <\scm-code>
        (eval (let*-\<gtr\>nested-lets exp) env)
      </scm-code>

      Since <scm|let> has already been implemented, all the <scm|let>
      expressions will surely be evaluated by <scm|eval>. So whenever the
      evaluator encounters a <scm|let*> expression, all it needs to do is
      transform that expression into a set of nested <scm|let> expressions
      using <scm|let*-\<gtr\>nested-lets>, the rest part of the evaluation is
      guaranteed to be handled by <scm|let>.
    </answer>
  </render-exercise>
</body>

<\initial>
  <\collection>
    <associate|font-base-size|11>
  </collection>
</initial>

<\references>
  <\collection>
    <associate|footnote-|<tuple|?|?|../../../../../../.TeXmacs/texts/scratch/no_name_23.tm>>
    <associate|footnote-*|<tuple|?|?>>
  </collection>
</references>