<TeXmacs|1.0.7.20>

<style|generic>

<\body>
  <\render-exercise|Exercise 1.13>
    <render-footnote|*|Creative Commons <image|<tuple|<#>|png>|1.20cm|0.42cm||>
    2013, Lawrence X. Amlord (<with|font|heiti|\<#989C\>\<#4E16\>\<#654F\>>,
    aka <with|font|heiti|\<#989C\> \<#5E8F\>>).>

    Prove that <math|Fib (n)> is the closest integer to
    <math|<frac*|\<phi\><rsup|n>|<sqrt|5>>>, where
    <math|\<phi\>=<frac*|<around*|(|1+<sqrt|5>|)>|2>>. Hint: Let
    <math|\<psi\>=<frac*|<around*|(|1-<sqrt|5>|)>|2>>. Use induction and the
    definition of the Fibonacci numbers (see section 1.2.2) to prove that
    <math|Fib <around*|(|n|)>=<frac*|<around*|(|\<phi\><rsup|n>-\<psi\><rsup|n>|)>|<sqrt|5>>>.

    <\proof>
      \;

      As indicated by the problem, in order to prove that <math|Fib (n)> is
      the closest integer to <math|<frac*|\<phi\><rsup|n>|<sqrt|5>>>, we had
      better prove\ 

      <\lemma*>
        <math|Fib <around*|(|n|)>=<frac*|<around*|(|\<phi\><rsup|n>-\<psi\><rsup|n>|)>|<sqrt|5>>>
        for all <math|n> greater than <math|0>.
      </lemma*>

      first.<new-line> <hspace|2ex>On the other hand, we see that the lemma
      <math|Fib <around*|(|n|)>=<frac*|<around*|(|\<phi\><rsup|n>-\<psi\><rsup|n>|)>|<sqrt|5>>>
      can be prove by means of mathematical induction.

      <strong|Base Cases:>

      <math|\<bullet\>> <math|<around*|(|n=0|)>> <math|Fib
      <around*|(|n|)>=<frac*|<around*|(|\<phi\><rsup|n>-\<psi\><rsup|n>|)>|<sqrt|5>>>
      is true for <math|n=0>, because <math|Fib
      <around*|(|0|)>=0=<frac*|<around*|(|\<phi\><rsup|0>-\<psi\><rsup|0>|)>|<sqrt|5>>>.

      <math|\<bullet\>> <math|<around*|(|n=1|)>> <math|Fib
      <around*|(|n|)>=<frac*|<around*|(|\<phi\><rsup|n>-\<psi\><rsup|n>|)>|<sqrt|5>>>
      also holds when <math|n=1>, for <math|Fib
      <around*|(|1|)>=1=<frac|<frac*|<around*|(|1+<sqrt|5>|)>|2>-<frac*|<around*|(|1-<sqrt|5>|)>|2>|<sqrt|5>>=<frac*|<around*|(|\<phi\><rsup|1>-\<psi\><rsup|1>|)>|<sqrt|5>>>.

      <strong|Inductive Step:><new-line> <hspace|2ex>Suppose
      <math|n\<geq\>2>, this proposition stands true in the case <math|n=k>
      as well as <math|n=k+1>, that is, <math|Fib
      <around*|(|k|)>=<frac*|<around*|(|\<phi\><rsup|k>-\<psi\><rsup|k>|)>|<sqrt|5>>>
      and <math|Fib <around*|(|k+1|)>=<frac*|<around*|(|\<phi\><rsup|k+1>-\<psi\><rsup|k+1>|)>|<sqrt|5>>>.
      We must show that this proposition also holds in the case where
      <math|n=k+2>, namely, <math|Fib <around*|(|k+2|)>=<frac*|<around*|(|\<phi\><rsup|k+2>-\<psi\><rsup|k+2>|)>|<sqrt|5>>>.<new-line>
      <hspace|2ex>Using the equation

      <\equation*>
        \<phi\><rsup|2>=\<phi\>+1 <around*|(|or \<psi\><rsup|2>=\<psi\>+1
        identically|)>
      </equation*>

      we can express the value of <math|Fib <around*|(|k+2|)>> in way akin to
      its predicessors':

      <\eqnarray*>
        <tformat|<table|<row|<cell|Fib <around*|(|k+2|)>>|<cell|=>|<cell|Fib
        <around*|(|k|)>+Fib<around*|(|k+1|)>>>|<row|<cell|>|<cell|=>|<cell|<frac*|<around*|(|\<phi\><rsup|k>-\<psi\><rsup|k>|)>|<sqrt|5>>+<frac*|<around*|(|\<phi\><rsup|k+1>-\<psi\><rsup|k+1>|)>|<sqrt|5>>>>|<row|<cell|>|<cell|=>|<cell|<frac|\<phi\><rsup|k>+\<phi\><rsup|k+1>-<around*|(|\<psi\><rsup|k>+\<psi\><rsup|k+1>|)>|<sqrt|5>>>>|<row|<cell|>|<cell|=>|<cell|<frac|\<phi\><rsup|k>*<around*|(|1+\<phi\>|)>-\<psi\><rsup|k>*<around*|(|1+\<psi\>|)>|<sqrt|5>>>>|<row|<cell|>|<cell|=>|<cell|<frac|\<phi\><rsup|k>*\<phi\><rsup|2>-\<psi\><rsup|k>*\<psi\><rsup|2>|<sqrt|5>>>>|<row|<cell|>|<cell|=>|<cell|<frac*|<around*|(|\<phi\><rsup|k+2>-\<psi\><rsup|k+2>|)>|<sqrt|5>>>>>>
      </eqnarray*>

      <hspace|3ex>Here we see that <math|Fib
      <around*|(|n|)>=<frac*|<around*|(|\<phi\><rsup|n>-\<psi\><rsup|n>|)>|<sqrt|5>>>
      also holds when <math|n=k+2>. This further completes the proof of the
      Lemma above by strong induction that <math|Fib
      <around*|(|n|)>=<frac*|<around*|(|\<phi\><rsup|n>-\<psi\><rsup|n>|)>|<sqrt|5>>>
      holds for all <math|n\<geq\>0>.<new-line> <hspace|2ex>Finally, notice
      that the value of <math|\<psi\><rsup|n>> keeps oscillating between the
      negtive number <math|\<phi\><rsup|>=<frac*|<around*|(|1-<sqrt|5>|)>|2>>
      and the positive integer <math|\<phi\><rsup|0>=1> while the sign of it
      changing back and forth as <math|n> varies. So, the term
      <math|\<psi\><rsup|n>> inside <math|Fib
      <around*|(|n|)>=<frac*|<around*|(|\<phi\><rsup|n>-\<psi\><rsup|n>|)>|<sqrt|5>>>
      becomes less and less significant as <math|n> going larger and larger.
      Therefore, <math|Fib (n)> is the closest integer to
      <math|<frac*|\<phi\><rsup|n>|<sqrt|5>>>.
    </proof>
  </render-exercise>
</body>

<\initial>
  <\collection>
    <associate|font-base-size|11>
  </collection>
</initial>

<\references>
  <\collection>
    <associate|footnote-|<tuple|?|?>>
    <associate|footnote-*|<tuple|?|?>>
    <associate|footnote-1|<tuple|1|1>>
    <associate|footnote-Creative Commons \ 2013, Lawrence R.
    Amlord|<tuple|?|1>>
    <associate|footnote-Creative Commons \ 2013, Lawrence R.
    Amlord.|<tuple|?|?>>
    <associate|footnr-1|<tuple|1|1>>
  </collection>
</references>