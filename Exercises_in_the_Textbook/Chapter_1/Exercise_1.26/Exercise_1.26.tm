<TeXmacs|1.0.7.19>

<style|generic>

<\body>
  <\render-exercise|Exercise 1.26>
    <render-footnote|*|Creative Commons <image|<tuple|<#>|png>|1.20cm|0.42cm||>
    2013, Lawrence R. Amlord(<with|font|heiti|\<#989C\>\<#4E16\>\<#654F\>>).>Louis
    Reasoner is having great difficulty doing exercise 1.24. His
    <scm|fast-prime?> test seems to run more slowly than his <scm|prime?>
    test. Louis calls his friend Eva Lu Ator over to help. When they examine
    Louis's code, they find that he has rewritten the <scm|expmod> procedure
    to use an explicit multiplication, rather than calling <scm|square>:

    <\scm-code>
      (define (expmod base exp m)

      \ \ (cond ((= exp 0) 1)

      \ \ \ \ \ \ \ \ ((even? exp)\ 

      \ \ \ \ \ \ \ \ \ (remainder (* (expmod base (/ exp 2) m)

      \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ (expmod base (/ exp 2)
      m))

      \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ m))

      \ \ \ \ \ \ \ \ (else\ 

      \ \ \ \ \ \ \ \ \ \ (remainder (* base (expmod base (- exp 1) m))

      \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ m))))
    </scm-code>

    ``I don't see what difference that could make,'' says Louis. ``I do.''
    says Eva. ``By writing the procedure like that, you have transformed the
    <math|\<Theta\> <around*|(|log n|)>> process into a <math|\<Theta\>
    <around*|(|n|)>> process.'' Explain.

    <\answer>
      In order to find out what cause that distinction between our original
      <scm|expmod> procedure and that of \ Louis, we'd better trace the
      process generated by both procedures.

      First, let's take a look at the process generated by our original
      <scm|expmod> procedure, for example, <scm|(expmod 3 4 5)>:

      <\scm-code>
        (expmod 3 4 5)

        (remainder (square (expmod 3 2 5))

        \ \ \ \ \ \ \ \ \ \ \ 5)

        (remainder (square (remainder (square (expmod 3 1 5))

        \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ 5))

        \ \ \ \ \ \ \ \ \ \ \ 5)

        (remainder (square (remainder (square (remainder (* 3

        \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ (expmod
        3 0 5))

        \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ 5))

        \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ 5))

        \ \ \ \ \ \ \ \ \ \ \ 5)

        (remainder (square (remainder (square (remainder (* 3 1)

        \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ 5))

        \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ 5))

        \ \ \ \ \ \ \ \ \ \ \ 5)

        (remainder (square (remainder (square (remainder 3 5))

        \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ 5))

        \ \ \ \ \ \ \ \ \ \ \ 5)

        (remainder (square (remainder (square 3)

        \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ 5))

        \ \ \ \ \ \ \ \ \ \ \ 5)

        (remainder (square (remainder 9 5))

        \ \ \ \ \ \ \ \ \ \ \ 5)

        (remainder (square 4)

        \ \ \ \ \ \ \ \ \ \ \ 5)

        (remainder 16 5)

        1
      </scm-code>

      Obviously, this is a linear recursive process. If I double the size of
      <scm|exp> in the initial expression, say <scm|(expmod 3 4 5)>, by means
      of successive squaring, what the interpreter has to do additionally is
      to add one more step both on the top and on the bottom of the process.
      So the number of steps grows logarithmically with the exponent, that
      is, <math|\<Theta\> <around*|(|log n|)>>.

      On the other hand, by breaking <scm|square> into an explicit
      multiplication in this case, Louis's procedure generates a tree
      recursion, as is shown in Figure <reference|Figure_1.26><\float|float|tbh>
        <big-figure|<label|Figure_1.26><math|Depth=log<rsub|2> n>
        <math|<choice|<tformat|<table|<row|<cell|<tree|<scm|(expmod 3 4
        5)>|<tree|<scm|(expmod 3 2 5)>|<tree|<scm|(expmod 3 1
        5)>|<tree|<scm|(expmod 3 0 5)>|1>>|<tree|<scm|(expmod 3 1
        5)>|<tree|<scm|(expmod 3 0 5)>|1>>>|<tree|<scm|(expmod 3 2
        5)>|<tree|<scm|(expmod 3 1 5)>|<tree|<scm|(expmod 3 0
        5)>|1>>|<tree|<scm|(expmod 3 1 5)>|<tree|<scm|(expmod 3 0
        5)>|1>>>>>>>>>>|The Tree-Recursive Process Generated in Computing
        Louis's <scm|expmod> Procedure>
      </float>. Note that this is a binary balanced tree with a depth of
      <math|log<rsub|2> n>, where <math|n> represents the scale of <scm|exp>
      in Louis's <scm|expmod> procedure. The amount of computation here will
      have to be multiplied by itself, even if I have only put an increment
      on <scm|exp>. Thus, the number of steps increases exponently, that is
      <math|\<Theta\> <around*|(|2<rsup|d>|)>>, where <math|d> is the depth
      of the tree in Figure <reference|Figure_1.26>.<\footnote>
        The number of steps can also be calculated by the amount of nodes in
        the tree, except for those who are leaves. Therefore, in a binary
        balanced tree whose depth is <math|log<rsub|2> n>, it takes up
        approximately <math|\<Theta\> <around*|(|2<rsup|d>|)>> steps for
        computation.\ 
      </footnote> Further more, we can express it in terms of <math|n>.

      <\eqnarray*>
        <tformat|<table|<row|<cell|\<Theta\>
        <around*|(|2<rsup|d>|)>>|<cell|=>|<cell|\<Theta\>
        <around*|(|2<rsup|log<rsub|2> n>|)>>>|<row|<cell|>|<cell|=>|<cell|\<Theta\>
        <around*|(|n|)>>>>>
      </eqnarray*>

      Hence, the order of growth here in the process generated by Louis's
      procedure is <math|\<Theta\> <around*|(|n|)>>.

      <\session|scheme|default>
        \;
      </session>
    </answer>
  </render-exercise>
</body>

<\references>
  <\collection>
    <associate|Figure_1.26|<tuple|1|2>>
    <associate|auto-1|<tuple|1|2>>
    <associate|auto-2|<tuple|2|2>>
    <associate|footnote-|<tuple|?|?>>
    <associate|footnote-*|<tuple|?|1>>
    <associate|footnote-1|<tuple|1|1>>
    <associate|footnr-1|<tuple|1|1>>
  </collection>
</references>

<\auxiliary>
  <\collection>
    <\associate|figure>
      <tuple|normal|The Tree-Recursive Process Generated in Computing Louis's
      <with|mode|<quote|prog>|prog-language|<quote|scheme>|expmod>
      Procedure|<pageref|auto-1>>
    </associate>
  </collection>
</auxiliary>