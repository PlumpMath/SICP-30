<TeXmacs|1.0.7.19>

<style|generic>

<\body>
  <\render-exercise|Exercise 1.29>
    <render-footnote|*|Creative Commons <image|<tuple|<#>|png>|1.20cm|0.42cm||>
    2013, Lawrence R. Amlord(<with|font|heiti|\<#989C\>\<#4E16\>\<#654F\>>).>Simpson's
    Rule is a more accurate method of numerical integration than the method
    illustrated above. Using Simpson's Rule, the integral of a function
    <math|f> between <math|a> and <math|b> is approximated as

    <\equation*>
      <frac|h|3>*<around*|[|y<rsub|0>+4*y<rsub|1>+2*y<rsub|2>+4*y<rsub|3>+2*y<rsub|4>+\<cdots\>+2*y<rsub|n-2>+4*y<rsub|n-1>+y<rsub|n>|]>
    </equation*>

    where <math|h= (b-a)/n>, for some even integer <math|n>, and
    <math|y<rsub|k>=f (a+k*h)>. (Increasing <math|n> increases the accuracy
    of the

    approximation.) Define a procedure that takes as arguments
    <math|f,a,b,and n> and returns the value of the integral, computed using
    Simpson's Rule. Use your procedure to integrate <scm|cube> between 0 and
    1 (with <math|n=100> and <math|n=1000>), and compare the results to those
    of the <scm|integral> procedure shown above.

    <\answer>
      In order to write down our procedure effectively, we are supposed to
      dig out the patterns submerged in the formula above. We start with
      analyzing its structure hoping that we could get some intuition for
      designing our procedure. The following questions will probably help us
      capture those underlying patterns:\ 

      <\enumerate-numeric>
        <item>Is the formula able to be decomposed into several parts among
        which we can use the <scm|sum> procedure?

        <item>What are the lower limit and the upper limit of the integral?

        <item>How to represent terms in the summation of <math|y> in our
        procedure?

        <item>What is the incremental step here?
      </enumerate-numeric>

      At a glance of the formula, one might immediately find out that it can
      be expressed with the product of <math|<frac|h|3>> and a summation
      which probably can be depicted by our <scm|sum> procedure. By observing
      the subscript of each term in the summation of <math|y>, we see that
      the lower limit of the intergral is <math|0> and the upper limit is
      <math|n>. Thus, by wishful thiking, we can put down the prototype of
      our procedure for approaching definite integral in terms of p <scm|sum>
      as follow:

      <\scm-code>
        (define (simp-int f a b n)

        \ \ (* (/ h 3)

        \ \ \ \ \ (sum simp-term 0 inc n)))
      </scm-code>

      Now what we have to do is to clarify two procedures which we left
      above: <scm|simp-term> and <scm|next>. Well, by looking at the patterns
      in coefficients of terms in the summation series

      <\eqnarray*>
        <tformat|<table|<row|<cell|Coefficient of
        y<rsub|k>>|<cell|=>|<cell|<choice|<tformat|<table|<row|<cell|1>|<cell|>|<cell|k=1
        or k=n>>|<row|<cell|4>|<cell|>|<cell|k is an odd
        integer>>|<row|<cell|2>|<cell|>|<cell|k is an even integer>>>>>>>>>
      </eqnarray*>

      and that <math|y<rsub|k>=f<around*|(|a+k*h|)>>, we are now able to
      express our <scm|simp-term> procedure as:

      <\scm-code>
        (define simp-term

        \ \ \ \ (lambda (k) (* (coef k)

        \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ (y k))))

        \ \ (define (coef k)

        \ \ \ \ (cond ((or (= k 0) (= k n)) 1)

        \ \ \ \ \ \ \ \ \ \ ((even? k) 2)

        \ \ \ \ \ \ \ \ \ \ (else 4)))

        \ \ (define (y k)

        \ \ \ \ (f (+ a (* k h))))
      </scm-code>

      The incremental step <scm|inc> here is obvious, just increase the
      subscript of <math|y<rsub|k>> by <math|1>:

      <\scm-code>
        (define inc (lambda (x) (+ x 1)))
      </scm-code>

      So far, we have create all the building blocks for our <scm|simp-int>
      procedure. All we have to do now is to assemble them into a complete
      module:

      <\scm-code>
        (define (simp-int f a b n)

        \ \ (define h (/ (- b a) n))

        \ \ (define simp-term

        \ \ \ \ (lambda (k) (* (coef k)

        \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ (y k))))

        \ \ (define (coef k)

        \ \ \ \ (cond ((or (= k 0) (= k n)) 1)

        \ \ \ \ \ \ \ \ \ \ ((even? k) 2)

        \ \ \ \ \ \ \ \ \ \ (else 4)))

        \ \ (define (y k)

        \ \ \ \ (f (+ a (* k h))))

        \ \ (define inc (lambda (x) (+ x 1)))

        \ \ (* (/ h 3)

        \ \ \ \ \ (sum simp-term 0 inc n)))
      </scm-code>

      Finally, let's use our procedure to integrate <scm|cube> between 0 and
      1 (with <math|n=100> and <math|n=1000>), and compare the results to
      those of the <scm|integral> procedure.

      <\scm-code>
        (simp-int cube 0 1 100.00)

        ;Value: .24999999999999992

        \;

        (simp-int cube 0 1 1000.00)

        ;Value: .2500000000000003
      </scm-code>

      Obiviously, comparing with that performance of the original
      <scm|integral> procedure, our procedure here integrates more
      accurately.
    </answer>
  </render-exercise>
</body>

<\references>
  <\collection>
    <associate|footnote-|<tuple|?|?>>
    <associate|footnote-*|<tuple|?|?>>
  </collection>
</references>