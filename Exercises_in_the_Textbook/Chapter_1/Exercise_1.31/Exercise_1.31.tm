<TeXmacs|1.0.7.19>

<style|generic>

<\body>
  <\render-exercise|Exercise 1.31>
    <render-footnote|*|Creative Commons <image|<tuple|<#>|png>|1.20cm|0.42cm||>
    2013, Lawrence R. Amlord(<with|font|heiti|\<#989C\>\<#4E16\>\<#654F\>>).>

    a. The <scm|sum> procedure is only the simplest of a vast number of
    similar abstractions that can be captured as higher-order
    procedures.<\footnote>
      The intent of exercises 1.31--1.33 is to demonstrate the expressive
      power that is attained by using an appropriate abstraction to
      consolidate many seemingly disparate operations. However, though
      accumulation and filtering are elegant ideas, our hands are somewhat
      tied in using them at this point since we do not yet have data
      structures to provide suitable means of combination for these
      abstractions. We will return to these ideas in section 2.2.3 when we
      show how to use <em|sequences> as interfaces for combining filters and
      accumulators to build even more powerful abstractions. We will see
      there how these methods really come into their own as a powerful and
      elegant approach to designing programs.
    </footnote> Write an analogous procedure called <scm|product> that
    returns the product of the values of a function at points over a given
    range. Show how to define <scm|factorial> in terms of product. Also use
    <scm|product> to compute approximations to <math|\<mathpi\>> using the
    formula<\footnote>
      This formula was discovered by the seventeenth-century English
      mathematician John Wallis.
    </footnote>

    <\equation*>
      <frac|\<mathpi\>|4>=<frac|2\<cdot\>4\<cdot\>4\<cdot\>6\<cdot\>6\<cdot\>8\<cdots\>|3\<cdot\>3\<cdot\>5\<cdot\>5\<cdot\>7\<cdot\>7\<cdots\>>
    </equation*>

    b. If your <scm|product> procedure generates a recursive process, write
    one that generates an iterative process. If it generates an iterative
    process, write one that generates a recursive process.

    <\answer>
      \;

      a. Much similar to the <scm|sum> procedure we've seen before, the
      procedure of <scm|product> here follows a abstraction of <em|product of
      a series>. Thus, we can obtain the procedure <scm|product> by
      performing only a minor adjustment on <scm|sum>. Both the recursive and
      iterative version are okay, but we discuss on the recursive version
      first.\ 

      However, in order to complete our procedure elegantly, we have to
      tackle two problems: What is the boundary of our test? How does the
      evaluator deal with it? Well, inspired by the <scm|sum> procedure, we
      know that the evaluation should terminate whenever <math|a> is greater
      than <math|b>. Further more, the evaluator should return <math|1> in
      this case.

      <\scm-code>
        (define (product term a next b)

        \ \ (if (\<gtr\> a b)

        \ \ \ \ \ \ 1

        \ \ \ \ \ \ (* (term a)

        \ \ \ \ \ \ \ \ \ (product term (next a) next b))))
      </scm-code>

      Therefore, we can immediately come up with the implementation for
      procedure <scm|factorial>:

      <\scm-code>
        (define (factorial n)

        \ \ (product identity 1 inc n))
      </scm-code>

      Before computing approximations to <math|\<mathpi\>>, let's first
      analyze those patterns submerged in the right part of the equation:

      <\equation*>
        <frac|2\<cdot\>4\<cdot\>4\<cdot\>6\<cdot\>6\<cdot\>8\<cdots\>|3\<cdot\>3\<cdot\>5\<cdot\>5\<cdot\>7\<cdot\>7\<cdots\>>
      </equation*>

      Note that the numerator here can evolve into:

      <\equation*>
        2\<cdot\><around*|(|4\<cdot\>6|)><rsup|2>\<cdot\>8
      </equation*>

      Similarly, we can view the denominator as:

      <\equation*>
        <around*|(|3\<cdot\>5\<cdot\>7\<cdot\>9\<cdots\>|)><rsup|2>
      </equation*>

      Hence, both the numerator and the denominator can be easily expressed
      in terms of the procedure <scm|product>. This eventually leads to the
      definition of the right part of the formula:

      <\scm-code>
        (define (pi-quotient a b)

        \ \ (define (pi-next x) (+ x 2))

        \ \ (define numer

        \ \ \ \ (product square (+ a 2) pi-next (- b 2)))

        \ \ (define denom

        \ \ \ \ (product square (+ a 1) pi-next (- b 1)))

        \ \ (/ (* a numer b) denom))
      </scm-code>

      Using this procedure, we can compute an approximation to
      <math|\<mathpi\>>:

      <\scm-code>
        (* 4 (pi-quotient 2 1000))
      </scm-code>

      b. Inspired by exercise 1.30, we can easily come up with an iterative
      version for procedure <scm|product>:

      <\scm-code>
        (define (product term a next b)

        \ \ (define (iter a result)

        \ \ \ \ (if (\<gtr\> a b)

        \ \ \ \ \ \ \ \ result

        \ \ \ \ \ \ \ \ (iter (next a) (* (term a) result))))

        \ \ (iter a 1))
      </scm-code>
    </answer>
  </render-exercise>
</body>

<\references>
  <\collection>
    <associate|footnote-|<tuple|?|?>>
    <associate|footnote-*|<tuple|?|1>>
    <associate|footnote-1|<tuple|1|1>>
    <associate|footnote-2|<tuple|2|1>>
    <associate|footnote-3|<tuple|3|?>>
    <associate|footnote-4|<tuple|4|?>>
    <associate|footnr-1|<tuple|1|1>>
    <associate|footnr-2|<tuple|2|1>>
    <associate|footnr-3|<tuple|3|?>>
    <associate|footnr-4|<tuple|4|?>>
  </collection>
</references>