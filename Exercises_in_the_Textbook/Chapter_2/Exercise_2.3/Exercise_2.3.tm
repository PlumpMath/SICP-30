<TeXmacs|1.0.7.19>

<style|generic>

<\body>
  <with|font-base-size|11|<\render-exercise|Exercise 2.3>
    <render-footnote|*|Creative Commons <image|<tuple|<#>|png>|1.20cm|0.42cm||>
    2013, Lawrence R. Amlord(<with|font|heiti|\<#989C\>\<#4E16\>\<#654F\>>).>

    Implement a representation for rectangles in a plane. (Hint: You may want
    to make use of exercise 2.2.) In terms of your constructors and
    selectors, create procedures that compute the perimeter and the area of a
    given rectangle. Now implement a different representation for rectangles.
    Can you design your system with suitable abstraction barriers, so that
    the same perimeter and area procedures will work using either
    representation?

    <\answer>
      \;

      Before setting out to implement a rectangle, let's ask ourselves a
      question: what is the most general practice for computing the perimeter
      and the area of a rectangle? Well, that would be:

      <\eqnarray*>
        <tformat|<table|<row|<cell|C>|<cell|=>|<cell|2*<around*|(|w+h|)>>>|<row|<cell|S>|<cell|=>|<cell|w*h>>>>
      </eqnarray*>

      where the <math|w> and <math|h> here stand for the width and heigh of a
      rectangle with respect.

      By wishful thinking, to evaluate the perimeter and area of a particular
      rectangle, all that we need to know is its width and heigh. Suppose we
      have a way of extracting the width and heigh of a given rectangle, say,
      the procedures <scm|width-rec> and <scm|heigh-rec>. We can express the
      idea for evaluating the perimeter and the area of a rectangle with
      these two procedures, despite of their implementation:\ 

      <\scm-code>
        (define (peri-rec r)

        \ \ (* (+ (width-rec r) (heigh-rec r)))

        \ \ \ \ \ 2))

        \;

        (define (area-rec r)

        \ \ (* (width-rec r) (heigh-rec r)))
      </scm-code>

      <subsubsection|The Usual Representation of a Rectangle>

      In order to implement the delayed procedure <scm|width-rec> and
      <scm|heigh-rec> above, we have to consider the concrete representation
      of a rectangle. A usual strategy would be by two adjcent segments, for
      example <math|m> and <math|l>, as is shown in Figure
      <reference|Exercise_2.3-Figure_1><\float|float|tbh>
        <big-figure|<label|Exercise_2.3-Figure_1><with|gr-mode|<tuple|group-edit|move>|gr-frame|<tuple|scale|1cm|<tuple|0.5gw|0.5gh>>|gr-geometry|<tuple|geometry|1par|0.6par>|gr-grid|<tuple|empty>|gr-grid-old|<tuple|cartesian|<point|0|0>|1>|gr-edit-grid-aspect|<tuple|<tuple|axes|none>|<tuple|1|none>|<tuple|10|none>>|gr-edit-grid|<tuple|empty>|gr-edit-grid-old|<tuple|cartesian|<point|0|0>|1>|gr-auto-crop|true|<graphics||<gr-group|<with|arrow-end|\<gtr\>|<line|<point|-1.47636592141818|-2.03503770339992>|<point|-1.47636592141818|2.96496229660008>>>|<with|arrow-end|\<gtr\>|<line|<point|-2.47636592141818|-1.03503770339992>|<point|3.52363407858182|-1.03503770339992>>>|<text-at|<math|O>|<point|-1.8333972747718|-1.36073554703003>>|<text-at|<math|A>|<point|-0.77636592141818|-0.33503770339992>>|<with|color|blue|<line|<point|-0.47636592141818|1.96496229660008>|<point|-0.47636592141818|-0.0350377033999201>>>|<with|color|red|<line|<point|-0.47636592141818|-0.0350377033999201>|<point|2.52363407858182|-0.0350377033999201>>>|<with|color|blue|<text-at|<math|l>|<point|-0.68246461172113|0.85473938351634>>>|<with|color|red|<text-at|<math|m>|<point|0.82363407858182|-0.33503770339992>>>|<with|color|red|dash-style|10|<line|<point|-0.47636592141818|1.96496229660008>|<point|2.52363407858182|1.96496229660008>>>|<text-at|<math|D>|<point|-0.77636592141818|2.06496229660008>>|<text-at|<math|y>|<point|-1.34580632358778|2.72750033073158>>|<with|color|blue|dash-style|10|<line|<point|2.52363407858182|1.96496229660008>|<point|2.52363407858182|-0.0350377033999201>>>|<text-at|<math|B>|<point|2.58486407858182|-0.33952500339992>>|<text-at|<math|x>|<point|3.22993120783172|-1.32629977510253>>|<text-at|<math|C>|<point|2.58197843630109|2.0565054901442>>>>>|Representing
        a Rectangle by Two Adjcent Line Segments>
      </float>. Thus, we can readily express this idea in Lisp as:

      <\scm-code>
        (define (make-rec m l) (cons m l))

        \;

        (define (m-rec r) (car r))

        \;

        (define (l-rec r) (cdr r))
      </scm-code>

      We can write the procedure <scm|width-rec> and <scm|heigh-rec> in terms
      of this representation with the help of <scm|length>, which is a
      procedure to measure the length of a segment.

      <\scm-code>
        (define (width-rec r) (length (m-rec r)))

        \;

        (define (heigh-rec r) (length (l-rec r)))
      </scm-code>

      The function of procedure <scm|length> here is to measure the length of
      a line segment and is defined to be:

      <\scm-code>
        (define (length s)

        \ \ (let ((dx (- (x-point (end-segment s))

        \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ (x-point (start-segment s))))

        \ \ \ \ \ \ \ \ (dy (- (y-point (end-segment s))

        \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ (y-point (end-segment s)))))

        \ \ \ \ (define (pythagoras x y)

        \ \ \ \ \ \ (sqrt (+ (square x) (square y))))

        \ \ \ \ (pythagoras dx dy)))
      </scm-code>

      <subsubsection|An Alternative Way for Representing a Rectangle>

      Besides the usual practice, there is another approach to representating
      a rectangle. Figure <reference|Exercise_2.3-Figure_2><\float|float|tbh>
        <big-figure|<label|Exercise_2.3-Figure_2><with|gr-mode|<tuple|group-edit|group-ungroup>|gr-frame|<tuple|scale|1cm|<tuple|0.5gw|0.5gh>>|gr-geometry|<tuple|geometry|1par|0.6par>|gr-grid|<tuple|empty>|gr-grid-old|<tuple|cartesian|<point|0|0>|1>|gr-edit-grid-aspect|<tuple|<tuple|axes|none>|<tuple|1|none>|<tuple|10|none>>|gr-edit-grid|<tuple|empty>|gr-edit-grid-old|<tuple|cartesian|<point|0|0>|1>|gr-color|red|gr-dash-style|10|gr-auto-crop|true|<graphics||<gr-group|<with|arrow-end|\<gtr\>|<line|<point|-1.98268620187856|-1.99576994311417>|<point|-1.98268620187856|3.00423005688583>>>|<text-at|<math|O>|<point|-2.30820620187856|-1.29576994311417>>|<with|arrow-end|\<gtr\>|<line|<point|-2.98268620187856|-0.99576994311417>|<point|3.01731379812144|-0.99576994311417>>>|<text-at|<math|A>|<point|-1.30501058341051|-0.28702540018521>>|<with|color|red|dash-style|10|<line|<point|-0.98268620187856|2.00423005688583>|<point|-0.98268620187856|0.00423005688583004>>>|<with|color|red|<line|<point|-0.98268620187856|0.00423005688583004>|<point|2.01731379812144|0.00423005688583004>>>|<with|color|red|<text-at|<math|h>|<point|-1.27944152321736|0.88977047228469>>>|<with|color|red|<text-at|<math|m>|<point|0.31731379812144|-0.29576994311417>>>|<with|color|blue|<line|<point|-0.98268620187856|2.00423005688583>|<point|2.01731379812144|2.00423005688583>>>|<text-at|<math|D>|<point|-1.36050072760948|2.09577655774573>>|<with|color|blue|dash-style|10|<line|<point|2.01731379812144|2.00423005688583>|<point|2.01731379812144|0.00423005688583004>>>|<text-at|<math|y>|<point|-1.86303413149888|2.77471226352692>>|<text-at|<math|B>|<point|2.01731379812144|-0.26595994311417>>|<text-at|<math|x>|<point|2.81731379812144|-1.29576994311417>>|<with|color|blue|<text-at|<math|n>|<point|0.41891453896018|2.11297790713057>>>|<text-at|<math|C>|<point|2.07565815584072|2.09577325042994>>>>>|Representing
        a Rectangle by Two Parallel Line Segments>
      </float> shows such a way geometrically by using two parallel line
      segments, say, <math|m> and <math|n>. This reveals a different way to
      to representing a rectangle in Lisp:

      <\scm-code>
        (define (make-rec m n) (cons m n))

        \;

        (define (m-rec r) (car r))

        \;

        (define (n-rec r) (cdr r))
      </scm-code>

      Using this representation, the procedure <scm|width-rec> and
      <scm|heigh-rec> should be implemented in a different way:

      <\scm-code>
        (define (width-rec r) (length (m-rec r)))

        \;

        (define (heigh-rec r)

        \ \ (let ((h (make-segment (start-segment (m-rec r))

        \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ (start-segment
        (n-rec r)))))

        \ \ \ \ (length h)))
      </scm-code>
    </answer>
  </render-exercise>>
</body>

<\references>
  <\collection>
    <associate|Exercise_2.3-Figure_1|<tuple|1|2>>
    <associate|Exercise_2.3-Figure_2|<tuple|2|2>>
    <associate|auto-1|<tuple|1|1>>
    <associate|auto-2|<tuple|1|2>>
    <associate|auto-3|<tuple|2|2>>
    <associate|auto-4|<tuple|2|2>>
    <associate|footnote-|<tuple|?|?>>
    <associate|footnote-*|<tuple|?|1>>
  </collection>
</references>

<\auxiliary>
  <\collection>
    <\associate|figure>
      <tuple|normal|Representing a Rectangle by Two Adjcent Line
      Segments|<pageref|auto-2>>

      <tuple|normal|Representing a Rectangle by Two Parallel Line
      Segments|<pageref|auto-4>>
    </associate>
    <\associate|toc>
      <with|par-left|<quote|3fn>|The Usual Representation of a Rectangle
      <datoms|<macro|x|<repeat|<arg|x>|<with|font-series|medium|<with|font-size|1|<space|0.2fn>.<space|0.2fn>>>>>|<htab|5mm>>
      <no-break><pageref|auto-1>>

      <with|par-left|<quote|3fn>|An Alternative Way for Representing a
      Rectangle <datoms|<macro|x|<repeat|<arg|x>|<with|font-series|medium|<with|font-size|1|<space|0.2fn>.<space|0.2fn>>>>>|<htab|5mm>>
      <no-break><pageref|auto-3>>
    </associate>
  </collection>
</auxiliary>