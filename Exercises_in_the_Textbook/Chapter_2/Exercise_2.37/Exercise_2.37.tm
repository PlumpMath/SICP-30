...
<\body>
  <\render-exercise|Exercise 2.37>
    ...
    <\answer>
      ...
      We learn in linear algebra that multiplying a matrix by a vector yeilds
      another vector, whose elements are the dot-product of the corresponding
      row of the matrix and the vector. This idea can be approached by
      <scm|map>ping a procedure which computes the dot-product of the
      corresponding row of the matrix and the specified vector to the matrix
      ...
      sequences into a sequence, all the second elements of the sequences,
      and so on, and returns a sequence of sequences. Notice that we can use
      <scm|accumulate-n> to depict this strategy by specifying a procedure
      which compose a sequence and setting its initial value\ 
      ...
      Finally, let's come to the operation of multiplying two matrices. We
      know mathematically that the product of two matrices <math|M> and
      <math|N> is still a matrix, whose entries are given by <em|dot-product>
      of the corresponding row of <math|M> and the corresponding column of
      <math|N>. This idea can be expressed in a different way in which we
      first obtian <math|N<rsup|T>>, the transposition of matrix <math|N>,
      then multiply <math|N<rsup|T>> by each row of <math|M> to obtain a
      sequence of sequences, and finally combine these sequences into the
      resulting matrix. And we do this by <scm|map>ping a procedure which
      computes thr product of <math|N<rsup|T>> by a row of <math|M> to matrix
      <math|M>
