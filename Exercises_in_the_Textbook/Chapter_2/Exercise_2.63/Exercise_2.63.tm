...
<\body>
  <\render-exercise|Exercise 2.63>
    ...
    steps required to convert a balanced tree with <math|n> elements to a
    list? If not, which one grows more slowly?

    <\answer>
      \;

      a. To see is to believe! We can determine their performance by running
      tests on the trees in figure 2.16:
      ...
      <\scm-code>
        (tree-\<gtr\>list-1 Tree-A)

        ;Value 20: (1 3 5 7 9 11)

        \;

        (tree-\<gtr\>list-2 Tree-A)

        ;Value 21: (1 3 5 7 9 11)

        \;

        (tree-\<gtr\>list-1 Tree-B)

        ;Value 22: (1 3 5 7 9 11)

        \;

        (tree-\<gtr\>list-2 Tree-B)

        ;Value 23: (1 3 5 7 9 11)

        \;

        (tree-\<gtr\>list-1 Tree-C)

        ;Value 24: (1 3 5 7 9 11)

        \;

        (tree-\<gtr\>list-2 Tree-C)
      ...
      b. Suppose we want to flatten a balanced binary tree of <math|n> nodes
      into a list using these two procedures seperately. We also assume that
      the total time costed by these two procedures are <math|T<rsub|1>
      <around*|(|n|)>> and <math|T<rsub|2> <around*|(|n|)>> respectfully.
      Since the <scm|tree-\<gtr\>list-1> procedure appends the resulting
      flattened left branch to the flattened right branch together with the
      entry of the tree. Thus, both the left branch and the right branch
      ...
      <\equation*>
        T<rsub|1> <around*|(|n|)>=\<Theta\> <around*|(|n*log n|)>
      </equation*>
      ...
      <\equation*>
        T<rsub|2> <around*|(|n|)>=2\<times\>T<rsub|2>
        <around*|(|<frac|n|2>|)>+\<Theta\> <around*|(|1|)>
      </equation*>

      Solve this equation we get

      <\equation*>
        T<rsub|2> <around*|(|n|)>=\<Theta\> <around*|(|n|)>
      </equation*>

      Therefore, the steps required by <scm|tree-\<gtr\>list-2> grows more
      slowly as the size of the problem increase.
    </answer>
  </render-exercise>
</body>
