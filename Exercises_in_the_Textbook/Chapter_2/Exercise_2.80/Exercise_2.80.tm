...
<\body>
  ...
  <|render-exercise>
    ...
    Define a generic predicate <scm|=zero?> that tests if its argument is
    ...
    <\answer>
      ...
      The implementation of generic predicate <scm|=zero?> is much similar to
      that of <scm|equ?> in exercise 2.79:<\footnote>
        Another strategy would be by calling the <scm|equ?> procedure with
        <scm|y> being set to <scm|0>:

        <\scm-code>
          (define (=zero? x) (equ? x 0))
        </scm-code>

        <hspace|3ex>Here we still adopt the practice we've taken in exercise
        2.79, simply for the reason that we are required to install the it in
        the generic arithmetic package.
      </footnote>
      ...
      <hspace|3ex>Once again, we begin by installing the <scm|=zero?>
      procedure into the <scm|scheme-number> package:
      ...
      <\scm-code>
        ...
        \ \ <with|prog-font-shape|italic|\<less\>other procedures in the
        package\<gtr\>>
      ...
      <hspace|3ex>The package that performs rational arithmetic after
      installing the <scm|=zero?> predicate appears to be:
      ...
      <hspace|3ex><hspace|3ex>Following the same way, we can add the
      <scm|=zero?> predicate into the package that handles complex
      numbers:<\footnote>
        We dispatch the equality test of a complex number with zero from
        outside world on to the corresponding <scm|=zero?> procedures in
        terms of rectangular form and polar form still for the sake of
        additivity. Now add the <scm|=zero?> predicate into both rectangular
        and polar packages:\ 
