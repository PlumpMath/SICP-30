...
<\body>
  <\render-exercise|Exercise 2.81>
    ...
    Louis Reasoner has noticed that <scm|apply-generic> may try to coerce the
    ...
    "coerce" arguments of each type to their own type. For example, in
    addition to the <scm|scheme-number-\<gtr\>complex> coercion shown above,
    he would do:
    ...
    <\scm-code>
      <with|prog-font-shape|italic|;; following added to Scheme-number
      package>

      (put 'exp '(scheme-number scheme-number)

      \ \ \ \ \ (lambda (x y) (tag (expt x y))))
      \ <with|prog-font-shape|italic|; using primitive> expt
    </scm-code>

    What happens if we call <scm|exp> with two complex numbers as arguments?

    b. Is Louis correct that something had to be done about coercion with
    arguments of the same type, or does <scm|apply-generic> work correctly as
    is?

    c. Modify <scm|apply-generic> so that it doesn't try coercion if the two
    ...
    <\answer>
      ...
      correspond to no operations in the table:
