...
<\body>
  <\render-exercise|Exercise 3.27>
    ...
    <with|font-shape|italic|Memoization> (also called
    <with|font-shape|italic|tabulation>) is a technique that enables a
    ...
    returns that value. Otherwise, it computes the new value in the ordinary
    way and stores this in the table. As an example of memoization, recall
    from section 1.2.2 the exponential process for computing Fibonacci
    numbers:
    ...
    Draw an environment diagram to analyze the computation of <scm|(memo-fib
    3)>. Explain why <scm|memo-fib> computes the <math|n>th Fibonacci number
    in a number of steps proportional to <math|n>. Would the scheme still
    work if we had simply defined <scm|memo-fib> to be <scm|(memoize fib)>?
