<TeXmacs|1.0.7.20>

<style|generic>

<\body>
  <\render-exercise|Exercise 3.32>
    <render-footnote|*|Creative Commons <image|<tuple|<#>|png>|1.20cm|0.42cm||>
    2013, Lawrence X. Amlord (<with|font|heiti|\<#989C\>\<#4E16\>\<#654F\>>,
    aka <with|font|heiti|\<#989C\> \<#5E8F\>>).>

    The procedures to be run during each time segment of the agenda are kept
    in a queue. Thus, the procedures for each segment are called in the order
    in which they were added to the agenda (first in, first out). Explain why
    this order must be used. In particular, trace the behavior of an and-gate
    whose inputs change from 0,1 to 1,0 in the same segment and say how the
    behavior would differ if we stored a segment's procedures in an ordinary
    list, adding and removing procedures only at the front (last in, first
    out).

    <\answer>
      \;

      Keeping the procedures of the same time segment in a queue guarantees
      us each function box derives its output from the most advanced inputs
      offered by the user. If we stored a segment's procedures in an ordinary
      list, the output a function box produces would be computed from the
      intermediate result and it may possibly be wrong. We shall see the
      underlying reason in tracing the behavior of an and-gate whose inputs
      change from 0,1 to 1,0 in the same segment.<new-line>
      <hspace|2ex>Figure <reference|Exercise_3.32-Figure_1><\float|float|tbh>
        <big-figure|<label|Exercise_3.32-Figure_1><with|gr-mode|<tuple|group-edit|group-ungroup>|gr-frame|<tuple|scale|1cm|<tuple|0.370012gw|0.529997gh>>|gr-geometry|<tuple|geometry|1par|0.6par>|gr-grid|<tuple|empty>|gr-grid-old|<tuple|cartesian|<point|0|0>|1>|gr-edit-grid-aspect|<tuple|<tuple|axes|none>|<tuple|1|none>|<tuple|10|none>>|gr-edit-grid|<tuple|empty>|gr-edit-grid-old|<tuple|cartesian|<point|0|0>|1>|gr-dash-style|10|gr-arrow-end|\<gtr\>|gr-auto-crop|true|<graphics||<with|arrow-end|\<gtr\>|dash-style|10|<spline|<point|2.70856|-0.0382822>|<point|3.4|0.0>|<point|3.79607090885038|-0.513262336287869>>>|<text-at|<scm|Output>|<point|1.59999524264561|-0.0999999999999999>>|<line|<point|0.6384159556756|-2.77555756156289e-17>|<point|1.45438960799132|-2.77555756156289e-17>>|<text-at|<scm|Input-1>|<point|-1.14561039200868|0.7>>|<with|magnify|0.511611092099832|<arc|<point|0.38261050606433|0.25580554604991>|<point|0.63841605211425|-2.77555756156289e-17>|<point|0.38261050606433|-0.25580554604992>>>|<gr-group|<with|dash-style|10|<arc|<point|1.33802407824501|-3.0>|<point|1.46432067733828|-3.3>|<point|1.83802407824501|-3.5>>>|<with|dash-style|10|<line|<point|1.33802407824501|-1.0>|<point|1.33802407824501|-3.0>>>|<with|dash-style|10|<line|<point|1.83802407824501|-3.5>|<point|8.338024078245|-3.5>>>|<with|dash-style|10|<arc|<point|1.33802407824501|-1.0>|<point|1.53802407824501|-0.60653343224368>|<point|1.83802407824501|-0.5>>>|<with|dash-style|10|<line|<point|1.83802407824501|-0.5>|<point|8.338024078245|-0.5>>>|<text-at|<scm|...>|<point|3.88881061754462|-2.57753471613271>>|<text-at|<scm|signal-value:
        0>|<point|1.63802407824501|-1.1>>|<text-at|<scm|(newline)>|<point|3.86836061754462|-1.89008302608166>>|<text-at|<scm|act-procs:
        ((lambda ()>|<point|1.63802407824501|-1.5>>|<text-at|<scm|(display
        'output)>|<point|3.88881061754462|-2.290815>>|<with|dash-style|10|<arc|<point|8.338024078245|-3.5>|<point|8.738024078245|-3.3>|<point|8.8114332593536|-3.00226551131102>>>|<text-at|<scm|(display
        (get-signal output))>|<point|3.85224284557981|-3.01127997211895>>|<with|dash-style|10|<line|<point|8.8239512551202|-1.00310655086652>|<point|8.8239486264657|-3.03512142346872>>>|<with|dash-style|10|<arc|<point|8.811434078245|-0.99142>|<point|8.7267659753213|-0.71624884243948>|<point|8.338024078245|-0.5>>>>|<with|magnify|0.511611092099832|<line|<point|0.38261050606433|0.25580554604991>|<point|-0.0266783676155399|0.25580554604991>|<point|-0.0266783676155399|-0.25580554604992>|<point|0.38261050606433|-0.25580554604992>>>|<line|<point|-0.0266803920086799|0.1>|<point|-0.54561039200868|0.1>|<point|-0.54561039200868|0.6>>|<with|arrow-end|\<gtr\>|dash-style|10|<spline|<point|-1.25437|0.761708>|<point|-1.6|1.0>|<point|-1.70730255324778|1.39175155443842>>>|<gr-group|<with|dash-style|10|<arc|<point|-2.50616|1.38674>|<point|-2.8024970234158|1.49257507606826>|<point|-3.01416523349649|1.89474467522159>>>|<with|dash-style|10|<line|<point|-3.0|2.5>|<point|-3.0|1.81006048475258>>>|<with|dash-style|10|<line|<point|-2.5|1.4>|<point|2.5|1.4>>>|<with|dash-style|10|<arc|<point|-2.50616|2.99542>|<point|-2.82366384442387|2.88958526260087>|<point|-2.99299841248842|2.3815815584072>>>|<with|dash-style|10|<line|<point|-2.5|3.0>|<point|2.5|3.0>>>|<text-at|<scm|signal-value:
        0>|<point|-2.7|2.3>>|<with|dash-style|10|<arc|<point|2.997212|1.81008>|<point|2.89137782775499|1.59840918110861>|<point|2.38337412356132|1.38674097102791>>>|<with|dash-style|10|<arc|<point|2.489208|2.99542>|<point|2.78554372271464|2.91075208360894>|<point|2.99721193279534|2.3815815584072>>>|<text-at|<scm|act-procs:
        (and-action-procedure)>|<point|-2.7|1.9>>|<with|dash-style|10|<line|<point|2.99721193279534|2.48741566344755>|<point|3.01002830565992|1.8>>>>|<line|<point|-0.0266740443243999|-0.1>|<point|-1.0289740443244|-0.1>>|<text-at|<scm|Input-2>|<point|-2.34561039200868|-0.2>>|<with|arrow-end|\<gtr\>|dash-style|10|<spline|<point|-2.45436|-0.138279>|<point|-3.0|-0.3>|<point|-3.10431273978039|-0.809597830400847>>>|<gr-group|<with|dash-style|10|<arc|<point|-4.60616|-2.41326>|<point|-4.9024970234158|-2.30742492393174>|<point|-5.11416523349649|-1.90525532477841>>>|<with|dash-style|10|<line|<point|-4.6|-2.4>|<point|0.4|-2.4>>>|<with|dash-style|10|<line|<point|-5.1|-1.3>|<point|-5.1|-1.98993951524742>>>|<with|dash-style|10|<arc|<point|-4.60616|-0.80458>|<point|-4.92366384442387|-0.91041473739913>|<point|-5.09299841248842|-1.4184184415928>>>|<with|dash-style|10|<line|<point|-4.6|-0.8>|<point|0.4|-0.8>>>|<text-at|<scm|signal-value:
        1>|<point|-4.8|-1.5>>|<with|dash-style|10|<arc|<point|0.897212|-1.98992>|<point|0.791377827754994|-2.20159081889139>|<point|0.283374123561318|-2.41325902897209>>>|<with|dash-style|10|<arc|<point|0.389208|-0.80458>|<point|0.685543722714645|-0.88924791639106>|<point|0.89721193279534|-1.4184184415928>>>|<text-at|<scm|act-procs:
        (and-action-procedure)>|<point|-4.8|-1.9>>|<with|dash-style|10|<line|<point|0.89721193279534|-1.31258433655245>|<point|0.91002830565992|-2.0>>>>>>|States
        of wires connected to an and-gate after initializing with inputs
        0,1.>
      </float> shows states of wires connected to an and-gate after
      initializing with 0,1.

      <\scm-code>
        (define the-agenda (make-agenda))

        (define and-gate-delay 3)

        \;

        (define input-1 (make-wire))

        (define input-2 (make-wire))

        (define output (make-wire))

        \;

        (probe 'output output)

        \;

        <\with|prog-font-shape|italic>
          output 0 \ New-value = 0

          ;Unspecified return value
        </with>

        \;

        (and-gate input-1 input-2 output)

        <with|prog-font-shape|italic|;Value: ok>

        \;

        (propagate)

        <with|prog-font-shape|italic|;Value: done>

        \;

        (set-signal! input-1 0)

        <with|prog-font-shape|italic|;Value: done>

        \;

        (set-signal! input-2 1)

        <with|prog-font-shape|italic|;Value: done>

        \;

        (propagate)

        <with|prog-font-shape|italic|;Value: done>

        \;

        (set-current-time! the-agenda 0)

        <with|prog-font-shape|italic|;Unspecified return value>
      </scm-code>

      There may be two possible kinds of intermediate inputs the and-gate
      would go through, say, <math|0,1\<rightarrow\>1,1\<rightarrow\>1,0> or
      <math|0,1\<rightarrow\>0,0\<rightarrow\>1,0>. This depends on which one
      of the inputs the user's choose to alter first.<new-line>
      <hspace|2ex>If the inputs to the and-gate were modified through the
      states of <math|0,1\<rightarrow\>1,1\<rightarrow\>1,0>, that is, the
      user first changed the signal value on <scm|input-1> to be 1, then
      revised the signal value on <scm|input-2> to be 0. The intermediate
      inputs 1,1 would stimulate a procedure to set the signal on
      <scm|output> to 1 after three time units from the begining and add this
      procedure into the agenda. Later the switch on inputs from 1,1 to 1,0
      arouse another procedure to set the signal on <scm|output> to 0 in the
      same segment. As procedures to be run during each time segment of the
      agenda were kept in an ordinary list, the procedure which computed the
      output signal from the most advanced inputs would be put ahead of the
      one which derived the output signal from the intermediate inputs in the
      same segment. Figure <reference|Exercise_3.32-Figure_2><\float|float|tbh>
        <big-figure|<label|Exercise_3.32-Figure_2><with|gr-mode|<tuple|group-edit|group-ungroup>|gr-frame|<tuple|scale|1cm|<tuple|0.450005gw|0.549995gh>>|gr-geometry|<tuple|geometry|1par|0.6par>|gr-grid|<tuple|empty>|gr-grid-old|<tuple|cartesian|<point|0|0>|1>|gr-edit-grid-aspect|<tuple|<tuple|axes|none>|<tuple|1|none>|<tuple|10|none>>|gr-edit-grid|<tuple|empty>|gr-edit-grid-old|<tuple|cartesian|<point|0|0>|1>|gr-point-style|round|gr-auto-crop|true|<graphics||<gr-group|<cline|<point|-3.0|2.82619>|<point|-3.0|-1.6>|<point|3.0|-1.6>|<point|3.0|2.82619>>|<line|<point|-1.7|1.92619>|<point|-1.7|-1.6>>|<text-at|<scm|3>|<point|-2.43459979013217|-0.43187364495184>>|<text-at|<scm|current-time>|<point|-5.70367802551643|0.93611>>|<with|arrow-end|\<gtr\>|<line|<point|-3.6|1.02619>|<point|-3.0|1.02619>>>|<with|dash-style|10|<line|<point|-1.7|-0.37381>|<point|3.0|-0.37381>>>|<line|<point|-2.95959979013217|0.73995135504816>|<point|3.0|0.72619>>|<line|<point|-2.95959979013217|1.33995135504816>|<point|3.0|1.32619>>|<text-at|<scm|0>|<point|-2.44999693830128|0.936926>>|<line|<point|-3.0|1.92619>|<point|3.0|1.92619>>|<text-at|<scm|Time>|<point|-2.65959979013217|1.51990775238637>>|<text-at|<scm|(lambda
        ()>|<point|-1.25959979013217|-0.86004864495184>>|<text-at|<scm|(lambda
        ()>|<point|-1.22231979013217|0.23995135504816>>|<text-at|<scm|(set-signal!
        output 1)>|<point|-0.95959979013217|-1.26004864495184>>|<text-at|<scm|Action>|<point|0.34040020986783|1.51990775238637>>|<text-at|<scm|The-agenda>|<point|-0.80825597705456|2.31115775238637>>|<with|arrow-end|\<gtr\>|<line|<point|3.027525|-1.39485>|<point|5.327525|-1.39485>|<point|5.327525|-2.19485>>>|<text-at|<scm|(set-signal!
        output 0)>|<point|-0.92231979013217|-0.16004864495184>>|<text-at|<scm|\<less\><with|prog-font-shape|italic|no
        action procedure>\<gtr\>>|<point|-1.160792|0.944741>>|<text-at|<scm|...)>|<point|4.7331897793145|-3.85189864495184>>|<text-at|call
        to <scm|probe>|<point|3.28520781888411|-1.29485>>|<text-at|<scm|(lambda
        ()>|<point|4.45190802875194|-2.58533864495184>>|<text-at|<scm|(newline)>|<point|4.75190802875194|-2.98533864495184>>|<text-at|<scm|(display
        'carry)>|<point|4.72441581888411|-3.37728387311125>>>>>|Contents of
        <scm|the-agenda> after changing the inputs in the sequence of
        <math|0,1\<rightarrow\>1,1\<rightarrow\>1,0>.>
      </float> presents us the agenda after changing the inputs in the
      sequence of <math|0,1\<rightarrow\>1,1\<rightarrow\>1,0>. Hence, when
      the simulator run the actions at the time segment 3, it first set the
      signal on <scm|output> to 0, then set it to 1. It was this mutation of
      changing the signal of <scm|output> from 0 to 1 triggered the
      <scm|probe> to print the new signal value of <scm|output>:

      <\scm-code>
        (set-signal! input-1 1)

        <with|prog-font-shape|italic|;Value: done>

        \;

        (set-signal! input-2 0)

        <with|prog-font-shape|italic|;Value: done>

        \;

        (propagate)

        \;

        <with|prog-font-shape|italic|output 3 \ New-value = 1>

        <with|prog-font-shape|italic|;Value: done>
      </scm-code>

      <hspace|3ex>However, if the user choose to modified the signal value on
      <scm|input-2> to be 0 first, in which the states of inputs the and-gate
      experienced were <math|0,1\<rightarrow\>0,0\<rightarrow\>1,0>. Since
      none of these inputs would make the and-gate derive its <scm|output>
      aside from 0, the agenda would remain empty through out the simulation
      and nothing more than <scm|<with|prog-font-shape|italic|'done>> would
      be printed when we allow the values to propogate:

      <\scm-code>
        (set-signal! input-2 0)

        <with|prog-font-shape|italic|;Value: done>

        \;

        (set-signal! input-1 1)

        <with|prog-font-shape|italic|;Value: done>

        \;

        (propagate)

        <with|prog-font-shape|italic|;Value: done>
      </scm-code>
    </answer>
  </render-exercise>
</body>

<\initial>
  <\collection>
    <associate|font-base-size|11>
  </collection>
</initial>

<\references>
  <\collection>
    <associate|Exercise_3.32-Figure_1|<tuple|1|?>>
    <associate|Exercise_3.32-Figure_2|<tuple|2|?>>
    <associate|auto-1|<tuple|1|1>>
    <associate|auto-2|<tuple|2|?>>
    <associate|footnote-|<tuple|?|?|../../../../../../.TeXmacs/texts/scratch/no_name_20.tm>>
    <associate|footnote-*|<tuple|?|1>>
  </collection>
</references>

<\auxiliary>
  <\collection>
    <\associate|figure>
      <tuple|normal|States of wires connected to an and-gate after
      initializing with inputs 0,1.|<pageref|auto-1>>

      <tuple|normal|Contents of <with|mode|<quote|prog>|prog-language|<quote|scheme>|font-family|<quote|rm>|the-agenda>
      after changing the inputs in the sequence of
      <with|mode|<quote|math>|0,1\<rightarrow\>1,1\<rightarrow\>1,0>.|<pageref|auto-2>>
    </associate>
  </collection>
</auxiliary>