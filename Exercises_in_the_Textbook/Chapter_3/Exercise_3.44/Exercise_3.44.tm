...
<\body>
  <\render-exercise|Exercise 3.44>
    ...
    <\answer>
      ...
      diagram in figure <reference|Exercise_3.44-Figure_1><\float|float|tbh>
      ...
      transfer. As shown in the diagram, this <scm|transfer> procedure acts
      correctly where there are multiple people concurrently transferring
      money among multiple accounts, even if we merely serialize the
      transactions on individual accounts.<new-line> <hspace|2ex>We see that
      the essential difference between the transfer operation here and the
      exchange operation above is that the amount of money been transferred
      between two account is specified by the account owner, which is
      irrelevant with the account balances involved, whereas the exchange
      operation swaps balances in two account based on their difference,
      whose value is highly hinged on the current balances of the accounts.
      This indicates that the transfer problem here is free from the
      consistency problem related to the account status, while the exchange
      problem above is inconsistency-prone, as we saw in exercise 3.43.
    </answer>
  </render-exercise>
</body>
...
<\references>
  <\collection>
    <associate|Exercise_3.44-Figure_1|<tuple|1|2>>
