...
<\body>
  <\render-exercise|Exercise 3.63>
    ...
    Louis Reasoner asks why the <scm|sqrt-stream> procedure was not written
    in the following more straightforward way, without the local variable
    <scm|guesses>:
    ...
    efficiency if our implementation of delay used only <scm|(lambda ()
    \<less\><with|prog-font-shape|italic|exp>\<gtr\>)> without using the
    optimization provided by <scm|memo-proc> (section 3.5.1)?
