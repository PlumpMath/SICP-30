<TeXmacs|1.0.7.20>

<style|generic>

<\body>
  <\render-exercise|Exercise 4.30>
    <render-footnote|*|Creative Commons <image|<tuple|<#>|png>|1.20cm|0.42cm||>
    2014, Lawrence X. Amlord (<with|font|heiti|\<#989C\>\<#4E16\>\<#654F\>>,
    aka <with|font|heiti|\<#989C\> \<#5E8F\>>).<new-line>
    <hspace|2.20ex>Email address: informlarry@gmail.com>

    Cy D. Fect, a reformed C programmer, is worried that some side effects
    may never take place, because the lazy evaluator doesn't force the
    expressions in a sequence. Since the value of an expression in a sequence
    other than the last one is not used (the expression is there only for its
    effect, such as assigning to a variable or printing), there can be no
    subsequent use of this value (e.g., as an argument to a primitive
    procedure) that will cause it to be forced. Cy thus thinks that when
    evaluating sequences, we must force all expressions in the sequence
    except the final one. He proposes to modify eval-sequence from section
    4.1.1 to use <scm|actual-value> rather than eval:

    <\scm-code>
      (define (eval-sequence exps env)

      \ \ (cond ((last-exp? exps) (eval (first-exp exps) env))

      \ \ \ \ \ \ \ \ (else (actual-value (first-exp exps) env)

      \ \ \ \ \ \ \ \ \ \ \ \ \ \ (eval-sequence (rest-exps exps) env))))
    </scm-code>

    a. Ben Bitdiddle thinks Cy is wrong. He shows Cy the <scm|for-each>
    procedure described in exercise 2.23, which gives an important example of
    a sequence with side effects:

    <\scm-code>
      (define (for-each proc items)

      \ \ (if (null? items)

      \ \ \ \ \ \ 'done

      \ \ \ \ \ \ (begin (proc (car items))

      \ \ \ \ \ \ \ \ \ \ \ \ \ (for-each proc (cdr items)))))
    </scm-code>

    He claims that the evaluator in the text (with the original
    <scm|eval-sequence>) handles this correctly:

    <\scm-code>
      <with|prog-font-shape|italic|;;; L-Eval input:>

      (for-each (lambda (x) (newline) (display x))

      \ \ \ \ \ \ \ \ \ \ (list 57 321 88))

      <with|prog-font-shape|italic|57>

      <with|prog-font-shape|italic|321>

      <with|prog-font-shape|italic|88>

      <with|prog-font-shape|italic|;;; L-Eval value:>

      <with|prog-font-shape|italic|done>
    </scm-code>

    Explain why Ben is right about the behavior of <scm|for-each>.

    b. Cy agrees that Ben is right about the <scm|for-each> example, but says
    that that's not the kind of program he was thinking about when he
    proposed his change to <scm|eval-sequence>. He defines the following two
    procedures in the lazy evaluator:

    <\scm-code>
      (define (p1 x)

      \ \ (set! x (cons x '(2)))

      \ \ x)

      \;

      (define (p2 x)

      \ \ (define (p e)

      \ \ \ \ e

      \ \ \ \ x)

      \ \ (p (set! x (cons x '(2)))))
    </scm-code>

    What are the values of <scm|(p1 1)> and <scm|(p2 1)> with the original
    <scm|eval-sequence>? What would the values be with Cy's proposed change
    to <scm|eval-sequence>?

    c. Cy also points out that changing <scm|eval-sequence> as he proposes
    does not affect the behavior of the example in part a. Explain why this
    is true.

    d. How do you think sequences ought to be treated in the lazy evaluator?
    Do you like Cy's approach, the approach in the text, or some other
    approach?

    <\answer>
      \;

      a. Ben is right about the behavior of <scm|for-each> because <scm|car>
      is a primitive operator in Scheme. Hence, thunks would be forced
      automatically before passed to it (in Ben's case, it is the thunk
      packaged <scm|items>). Besides, <scm|proc> will also be automatically
      forced before applied. Note that our lazy evaluator delays arguments in
      an expression rather than expressions in a sequence. So long as the
      values of its arguments are presented, any expression of a sequence can
      be completely evaluated by <scm|eval>, no matter its value is used or
      not. Thus the expression <scm|(proc (car items))> will be utterly
      evaluated in every call to <scm|for-each>, which surely leads to the
      interaction above.

      b. Evaluating <scm|(p1 1)> and <scm|(p2 1)> in the original
      <scm|eval-sequence> involves the following interaction:

      <\scm-code>
        <with|prog-font-shape|italic|;;; L-Eval input:>

        (p1 1)

        <\with|prog-font-shape|italic>
          ;;; L-Eval value:

          (1 2)
        </with>

        \;

        <with|prog-font-shape|italic|;;; L-Eval input:>

        (p2 1)

        <\with|prog-font-shape|italic>
          ;;; L-Eval value:

          1
        </with>
      </scm-code>

      <hspace|3ex>The value of <scm|(p1 1)> remains the same whereas that of
      <scm|(p2 1)> varies in Cy's proposed change to <scm|eval-sequence>:

      <\scm-code>
        <with|prog-font-shape|italic|;;; L-Eval input:>

        (p1 1)

        <\with|prog-font-shape|italic>
          ;;; L-Eval value:

          (1 2)
        </with>

        \;

        <with|prog-font-shape|italic|;;; L-Eval input:>

        (p2 1)

        <\with|prog-font-shape|italic>
          ;;; L-Eval value:

          (1 2)
        </with>
      </scm-code>

      c. Because <scm|proc> as well as the thunk contains <scm|items><scm|>
      inside <scm|(proc (car items))> is automatically forced by the lazy
      evaluator, no matter evaluated by <scm|eval> or <scm|actual-value>.

      d. Personally, I prefer the approach in the text to deal with sequence
      in the lazy evaluator. As I mentioned in part a, so long as the values
      of its arguments are presented, any expression of a sequence can be
      completely evaluated by <scm|eval>, no matter its value is used or not.
      Hence, it's excessive to exploit <scm|actual-value> to evaluate
      expressions in the sequence. Besides, this modification can arise
      confusion in the presence of side effect, as the interaction in part c
      shows.
    </answer>
  </render-exercise>
</body>

<\initial>
  <\collection>
    <associate|font-base-size|11>
  </collection>
</initial>

<\references>
  <\collection>
    <associate|footnote-|<tuple|?|?|../../../../../../.TeXmacs/texts/scratch/no_name_23.tm>>
    <associate|footnote-*|<tuple|?|?|../../../../../../.TeXmacs/texts/scratch/no_name_23.tm>>
  </collection>
</references>