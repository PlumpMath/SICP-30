<TeXmacs|1.0.7.20>

<style|generic>

<\body>
  <\render-exercise|Exercise 4.9>
    <render-footnote|*|Creative Commons <image|<tuple|<#>|png>|1.20cm|0.42cm||>
    2014, Lawrence X. Amlord (<with|font|heiti|\<#989C\>\<#4E16\>\<#654F\>>,
    aka <with|font|heiti|\<#989C\> \<#5E8F\>>).<new-line>
    <hspace|2.20ex>Email address: informlarry@gmail.com>

    Many languages support a variety of iteration constructs, such as
    <scm|do>, <scm|for>, <scm|while>, and <scm|until>. In Scheme, iterative
    processes can be expressed in terms of ordinary procedure calls, so
    special iteration constructs provide no essential gain in computational
    power. On the other hand, such constructs are often convenient. Design
    some iteration constructs, give examples of their use, and show how to
    implement them as derived expressions.

    <\answer>
      \;

      <scm|Do> is an iteration construct that has the form<\footnote>
        R5RS\ 
      </footnote>

      <\scm-code>
        (do ((\<less\><math|variable<rsub|1>>\<gtr\>
        \<less\><math|init<rsub|1>>\<gtr\>
        \<less\><math|step<rsub|1>>\<gtr\>)

        \ \ \ \ \ (\<less\><math|variable<rsub|2>>\<gtr\>
        \<less\><math|init<rsub|2>>\<gtr\>
        \<less\><math|step<rsub|2>>\<gtr\>)

        \ \ \ \ \ ...

        \ \ \ \ \ (\<less\><math|variable<rsub|n>>\<gtr\>
        \<less\><math|init<rsub|n>>\<gtr\>
        \<less\><math|step<rsub|n>>\<gtr\>))

        \ \ \ \ (\<less\><with|prog-font-shape|italic|test>\<gtr\>
        \<less\><math|expression<rsub|1>>\<gtr\>
        \<less\><math|expression<rsub|2>>\<gtr\> ...
        \<less\><math|expression<rsub|n>>\<gtr\>)

        \ \ \ \ \<less\><with|prog-font-shape|italic|command>\<gtr\> ...)
      </scm-code>

      It specifies a set of variables to be bound, how they are to be
      initialized at the start, and how they are to be updated on each
      iteration. When a termination condition is met, the loop exits after
      evaluating the <scm|\<less\><with|prog-font-shape|italic|expression>\<gtr\>>s.<new-line>
      <hspace|2ex><scm|Do> expressions are evaluated as follows: The
      <scm|\<less\><with|prog-font-shape|italic|init>\<gtr\>> expressions are
      evaluated (in some unspecified order), the
      <scm|\<less\><with|prog-font-shape|italic|variable>\<gtr\>>s are bound
      to fresh locations, the results of the
      <scm|\<less\><with|prog-font-shape|italic|init>\<gtr\>> expressions are
      stored in the bindings of the <scm|\<less\><with|prog-font-shape|italic|variable>\<gtr\>>s,
      and then the iteration phase begins.<new-line> <hspace|2ex>Each
      iteration begins by evaluating <scm|\<less\><with|prog-font-shape|italic|test>\<gtr\>>;
      if the result is false, then the <scm|\<less\><with|prog-font-shape|italic|command>\<gtr\>>
      expressions are evaluated in order for effect, the
      <scm|\<less\><with|prog-font-shape|italic|step>\<gtr\>> expressions are
      evaluated in some unspecified order, the
      <scm|\<less\><with|prog-font-shape|italic|variable>\<gtr\>>s are bound
      to fresh locations, the results of the
      <scm|\<less\><with|prog-font-shape|italic|step>\<gtr\>>s are stored in
      the bindings of the <scm|\<less\><with|prog-font-shape|italic|variable>\<gtr\>>s,
      and the next iteration begins.<new-line> <hspace|2ex>If
      <scm|\<less\><with|prog-font-shape|italic|test>\<gtr\>> evaluates to a
      true value, then the <scm|\<less\><with|prog-font-shape|italic|expression>\<gtr\>>s
      are evaluated from left to right and the value(s) of the last
      <scm|\<less\><with|prog-font-shape|italic|expression>\<gtr\>> is(are)
      returned. If no <scm|\<less\><with|prog-font-shape|italic|expression>\<gtr\>>s
      are present, then the value of the <scm|do> expression is unspecified
      in standard Scheme; we have chosen here to make it false. For example

      <\scm-code>
        (let ((x '(1 3 5 7 9)))

        \ \ (do ((x x (cdr x))

        \ \ \ \ \ \ \ (sum 0 (+ sum (car x))))

        \ \ \ \ \ \ ((null? x) sum)))
      </scm-code>

      returns 25. We can reduce the problem of evaluating this <scm|do>
      expression to the problem of evaluating the following expression
      involving <scm|begin>, <scm|define> and <scm|if> expressions:

      <\scm-code>
        (let ((x '(1 3 5 7 9)))

        \ \ (begin (define (iter x sum)

        \ \ \ \ \ \ \ \ \ \ \ (if (null? x)

        \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ sum

        \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ (iter (cdr x) (+ sum (car x)))))

        \ \ \ \ \ \ \ \ \ (iter x 0)))
      </scm-code>

      <hspace|3ex>We include syntax
    </answer>
  </render-exercise>
</body>

<\initial>
  <\collection>
    <associate|font-base-size|11>
  </collection>
</initial>

<\references>
  <\collection>
    <associate|footnote-|<tuple|?|?|../../../../../../.TeXmacs/texts/scratch/no_name_23.tm>>
    <associate|footnote-*|<tuple|?|?|../../../../../../.TeXmacs/texts/scratch/no_name_23.tm>>
    <associate|footnote-1|<tuple|1|?|../../../../../../.TeXmacs/texts/scratch/no_name_23.tm>>
    <associate|footnr-1|<tuple|1|?|../../../../../../.TeXmacs/texts/scratch/no_name_23.tm>>
  </collection>
</references>