<TeXmacs|1.0.7.19>

<style|generic>

<\body>
  <\render-exercise|Problem 3>
    <render-footnote|*|Creative Commons <image|<tuple|<#>|png>|1.20cm|0.42cm||>
    2013, Lawrence R. Amlord(<with|font|heiti|\<#989C\>\<#4E16\>\<#654F\>>).>

    Define a procedure <scm|test-strategy> that tests two strategies by
    playing a specified number of simulated Twenty-One games using the two
    strategies. <scm|Test-strategy> should return the number of games that
    were won by the player (and thus lost by the house). For example,

    <\scm-code>
      (test-strategy (stop-at 16) (stop-at 15) 10)
    </scm-code>

    should play ten games of Twenty-One, using the value returned by
    <scm|(stop-at 16)> as the player's strategy and the value of
    <scm|(stop-at 15)> as the house strategy. It should return a non-negative
    integer indicating how many games were won by the player. Turn in a
    listing of your procedure and some sample results.

    <\answer>
      \;

      As is often the case, there are two ways to implement the procedure
      <scm|test-strategy>, the recursive version and the iterative version.

      As a way for starting, lat's first come to deal with the recursive one.
      Remember the principle to design a recursive procedure:\ 

      <\enumerate-roman>
        <item>Wishful thinking.

        <item>Decompose the problem.

        <item>Identify non-decomposable problems.
      </enumerate-roman>

      By wishful thinking, suppose that we can solve the problem, but only
      for versions of the problem smaller than the current one. That is the
      smaller version of <scm|test-strategy> just plays one less round of
      game compare to the original one.

      Given that assumpsion, the second stage proceeds to convert that
      simplified solution to the desired solution. We saw this with
      <scm|test-strategy>, where we combine the solution to a smaller version
      of <scm|test-strategy>, plus the result of the current round, to create
      the full version of <scm|test-strategy>.

      <\scm-code>
        (define (test-strategy player-strategy house-strategy n)

        \ \ (+ \<less\>current-result\<gtr\>

        \ \ \ \ \ (test-strategy player-strategy house-strategy (- n 1))))
      </scm-code>

      Now, it time to consider the simplest case where I can solve directly,
      without using wishful thinking. In the case of <scm|test-strategy>,
      that is just knowing that when it comes to play only a single round of
      the game, number of games that were won by the player is simply the
      result of the current round.

      <\scm-code>
        (if (\<less\> n 2)

        \ \ \ \ \<less\>current-result\<gtr\>

        \ \ \ \ (...))
      </scm-code>

      However, there is still a few problems left to tackle: how to represent
      the result of the current round? And how can we denote the number of
      games that were won by the player? Well, note what the game convey to
      us about its rule: the procedure <scm|twenty-one> returns <math|1> if
      the player wins the simulated game and <math|0> if the house wins.
      Hence, this solve the first problem. On the other hand,
      <scm|test-strategy> should return the number of games that were won by
      the player. This indicates that the number of the game that won by the
      player is simply the result accumulated from every round of the game.
      Therefore, we can denote the result of the current round by setting a
      local variable, namely <scm|pw>, and we the game ends, the value of
      <scm|pw> is just the the number of games that were won by the player.

      After assembling, the building blocks, we have craft, our
      <scm|test-strategy> procedure turns out to be:

      <\scm-code>
        (define (test-strategy player-strategy house-strategy n)

        \ \ (let (pw (twenty-one player-strategy house-strategy))

        \ \ \ \ (if (\<less\> n 2)

        \ \ \ \ \ \ \ \ pw

        \ \ \ \ \ \ \ \ (+ pw

        \ \ \ \ \ \ \ \ \ \ \ (test-strategy player-strategy house-strategy
        (- n 1))))))
      </scm-code>

      Now, let's come to design the iterative version of procedure
      <scm|test-strategy>. To do this, we'd better figure out a way to
      accumalate partial answer, that is, how many variables are needed to
      depict the computation. Well, in this process, we have to keep track of
      4 things, the current round, the number of games that were won by the
      player, the result of the current round and the total number of games.
      Suppose that both of the opponents have played <math|10> rounds of
      games in which the player took 6.

      As is shown in Table <reference|Problem_3-Table_1><\float|float|tbh>
        <big-table|<label|Problem_3-Table_1><with|gr-mode|<tuple|group-edit|props>|gr-frame|<tuple|scale|1cm|<tuple|0.5gw|0.5gh>>|gr-geometry|<tuple|geometry|1par|0.6par>|gr-grid|<tuple|empty>|gr-grid-old|<tuple|cartesian|<point|0|0>|1>|gr-edit-grid-aspect|<tuple|<tuple|axes|none>|<tuple|1|none>|<tuple|10|none>>|gr-edit-grid|<tuple|empty>|gr-edit-grid-old|<tuple|cartesian|<point|0|0>|1>|gr-auto-crop|true|<graphics||<gr-group|<gr-group|<with|color|red|<cline|<point|-4.85154712131234|0.15053308109538>|<point|-3.00296004762534|0.15053247784098>|<point|-3.00296004762534|-0.26871940732901>|<point|-4.85155113110199|-0.26871940732901>>>|<with|color|red|<text-at|<scm|(+
        round 1)>|<point|-4.75163712131234|-0.14530691890462>>>>|<cline|<point|-2.35661|2.14749>|<point|3.1|2.14748974732107>|<point|3.1|-2.11597102791375>|<point|-2.35661132424924|-2.11597102791375>>|<with|arrow-end|\<gtr\>|color|red|<spline|<point|-1.79817|0.33012>|<point|-3.0|0.7>|<point|-3.99952043921154|0.160785156766768>>>|<with|arrow-end|\<gtr\>|color|red|<spline|<point|-3.00468|-0.0932167>|<point|-1.86167151739648|-0.0932166953300701>>>|<with|color|red|<carc|<point|-1.82884|-0.241384>|<point|-1.62884|-0.241384>|<point|-1.82884|-0.041384>>>|<with|color|red|<carc|<point|-1.82539036769494|0.108587798496832>|<point|-1.62539036769494|0.108587798496832>|<point|-1.82539036769494|0.308587798496832>>>|<with|color|blue|<cspline|<point|-0.446484|0.203119>|<point|0.753516|0.01899386913645>|<point|1.953516|0.203119>|<point|0.753516|0.403119>>>|<with|color|magenta|<carc|<point|1.59822508075689|-1.97123>|<point|1.79822508075689|-1.97123>|<point|1.59822508075689|-1.67123>>>|<with|arrow-end|\<gtr\>|color|magenta|<spline|<point|2.96436|-2.8449>|<point|2.5|-2.7>|<point|2.4|-2.0>|<point|1.84252215901574|-1.93473012303215>>>|<with|color|blue|<carc|<point|1.59435|-0.2957744362268>|<point|1.79435|-0.2957744362268>|<point|1.59435|0.00422556377320001>>>|<with|arrow-end|\<gtr\>|color|blue|<spline|<point|2.98553|-0.283718>|<point|1.82135533800767|-0.304884905410769>>>|<text-at|<tabular*|<tformat|<table|<row|<cell|Round>|<cell|Round
        Result>|<cell|Player Won>|<cell|N>|<cell|>>|<row|<cell|1>|<cell|0>|<cell|0>|<cell|10>|<cell|>>|<row|<cell|2>|<cell|1>|<cell|0>|<cell|10>|<cell|>>|<row|<cell|3>|<cell|1>|<cell|1>|<cell|10>|<cell|>>|<row|<cell|4>|<cell|0>|<cell|2>|<cell|10>|<cell|>>|<row|<cell|5>|<cell|1>|<cell|2>|<cell|10>|<cell|>>|<row|<cell|6>|<cell|1>|<cell|3>|<cell|10>|<cell|>>|<row|<cell|7>|<cell|0>|<cell|4>|<cell|10>|<cell|>>|<row|<cell|8>|<cell|0>|<cell|4>|<cell|10>|<cell|>>|<row|<cell|9>|<cell|1>|<cell|4>|<cell|10>|<cell|>>|<row|<cell|10>|<cell|1>|<cell|5>|<cell|10>|<cell|>>|<row|<cell|11>|<cell|>|<cell|6>|<cell|>|<cell|>>>>>|<point|-2.2566940071438|-0.0464016404286281>>|<with|arrow-end|\<gtr\>|color|blue|<spline|<point|1.60969|0.308953>|<point|3.4|0.8>|<point|4.95404484720201|0.118451514750628>>>|<gr-group|<with|color|blue|<cline|<point|2.9902573501786|0.11007799444371>|<point|7.23704524407991|0.11007739118931>|<point|7.23704524407991|-0.34836618600344>|<point|2.99025334038895|-0.34836618600344>>>|<with|color|blue|<text-at|<scm|(+
        player-won round-result)>|<point|3.0901673501786|-0.18576200555629>>>>>|<gr-group|<with|color|magenta|<cline|<point|2.98395455952392|-2.64470009128192>|<point|4.07665461111805|-2.64469837280064>|<point|4.07665461111805|-2.96600410107157>|<point|2.98395054973427|-2.96600410107157>>>|<with|color|magenta|<text-at|answer|<point|3.08386455952392|-2.86609009128192>>>>>>|The
        Evoluation of Process in Evaluating <scm|(test-strategy (stop-at 16)
        (stop-at 15) 10) >>
      </float>, the rule for getting the next value of round is simply
      increasing it by 1. In order to figure out how many rounds have been
      won by the player, just add the current round result to the record that
      keeps the number of rounds formerly won by the player. Further more, as
      is indicated by the table, the number of rounds that won by the player
      reveals to be our final result when the value exceeds N, here it is
      <math|10>. Finally, we need a way to start, that's easy, just set the
      initial value of Round, Round Result and Player Won to 1, 0 and 0 with
      respect, as is shown in Table <reference|Problem_3-Table_1>.

      Having done the analysis above, now we can write the procedure
      <scm|test-strategy> in a quite clear way:

      <\scm-code>
        (define (test-strategy player-strategy house-strategy n)

        \ \ (define (play round pw)

        \ \ \ \ (if (\<gtr\> round n)

        \ \ \ \ \ \ \ \ pw

        \ \ \ \ \ \ \ \ (play (+ round 1)

        \ \ \ \ \ \ \ \ \ \ \ \ \ \ (+ pw

        \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ (twenty-one player-strategy
        house-strategy)))))

        \ \ (play 1 0))
      </scm-code>
    </answer>
  </render-exercise>
</body>

<\references>
  <\collection>
    <associate|Problem_3-Table_1|<tuple|1|?>>
    <associate|auto-1|<tuple|1|?>>
    <associate|auto-2|<tuple|2|?>>
    <associate|footnote-|<tuple|?|?>>
    <associate|footnote-*|<tuple|?|?>>
  </collection>
</references>

<\auxiliary>
  <\collection>
    <\associate|table>
      <tuple|normal||<pageref|auto-1>>

      <tuple|normal||<pageref|auto-2>>
    </associate>
  </collection>
</auxiliary>